...
| `dv_wallet_id` | text | The temporary deposit address generated by CryptAPI |
| `pay_url` | text | QR code URL (coin purchases) or `entrance_tribute:Name` (tribute) |
...
- **No API keys stored client-side** — All CryptAPI calls happen server-side
- **Auth required** — `/api/crypto/create` and `/api/tribute/crypto` require a valid Supabase session
- **Idempotent webhooks** — Webhook checks if the order is already completed before crediting coins, preventing double-crediting
...
- **Webhook IP allowlisting** — CryptAPI webhooks are validated against known CryptAPI server IPs
