...
export function initDashboard() {
    ...
    // Recalculate consistency for all users on dashboard load
    fetch('/api/admin/backfill-consistency', { method: 'POST' })
        .then(r => r.json())
        .then(d => console.log('[BACKFILL] consistency recalculated:', d))
...
function subscribeToDashboardTaskUpdates() {
    ...
    // Global chat sidebar updates handled by chats-admin-live channel in page.tsx
}
        ...
        const html = transactions.map(tx => {
            try {
                ...
                const typeMap: Record<string, { label: string; color: string; border: string }> = {
                    ...
                    PAYWALL_TRIBUTE:        { label: 'FORCED TRIBUTE', color: 'rgba(197,160,89,0.8)',    border: 'rgba(197,160,89,0.25)' },
                    PAYWALL_TRIBUTE_CRYPTO: { label: 'TRIBUTE · CRYPTO', color: 'rgba(197,160,89,0.8)', border: 'rgba(197,160,89,0.25)' },
                    COIN_PURCHASE:          { label: 'COIN PURCHASE',  color: 'rgba(120,160,255,0.8)',   border: 'rgba(120,160,255,0.2)' },
                };
                const meta = typeMap[tx.type] || { label: tx.type || 'PURCHASE', color: 'rgba(255,255,255,0.4)', border: 'rgba(255,255,255,0.1)' };

                return `
                    <div class="exchequer-row" style="background:rgba(255,255,255,0.02);border:1px solid ${meta.border};border-radius:8px;padding:12px 14px;flex-direction:column;align-items:flex-start;gap:5px;">
                        <div style="display:flex;justify-content:space-between;align-items:center;width:100%;">
                            <div style="font-family:Rajdhani,sans-serif;font-size:0.38rem;color:${meta.color};letter-spacing:3px;">${meta.label}</div>
                            ${amountStr ? `<div style="font-family:Rajdhani,sans-serif;font-size:0.95rem;font-weight:700;color:#fff;">${amountStr}</div>` : ''}
                        </div>
                        <div style="font-family:Rajdhani,sans-serif;font-size:0.65rem;color:rgba(255,255,255,0.7);letter-spacing:1px;">${tx.name || ''}</div>
                        ${tx.reason ? `<div style="font-family:Rajdhani,sans-serif;font-size:0.7rem;color:rgba(255,255,255,0.35);font-style:italic;">"${tx.reason}"</div>` : ''}
