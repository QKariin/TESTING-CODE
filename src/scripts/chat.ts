...
export async function renderChat(messages: any[]) {
    ...
    const messagesHtmlArray = await Promise.all(visibleMessages.map(async (m) => {
        ...
        if (originalMsg) {
            ...
            // A. TASK REVIEW CARD
            else if (originalMsg.startsWith('TASK_REVIEW_CARD::')) {
                try {
                    ...
                    const commentLine = d.comment
                        ? `<div style="font-family:Rajdhani,sans-serif;font-size:0.78rem;color:rgba(255,255,255,0.55);font-style:italic;margin-top:6px;line-height:1.4;">"${DOMPurify.sanitize(d.comment)}"</div>`
                        : '';
                    ...
                    <div style="width:min(88%,340px);margin:0 auto;">
                        <div style="border-radius:12px;overflow:hidden;background:#0c0c0c;border:1px solid ${accentBorder};">
                            ...
                            <div style="position:relative;display:flex;gap:14px;padding:12px 16px;">
                                <div style="position:absolute;top:10px;right:14px;font-family:Rajdhani,sans-serif;font-size:0.58rem;color:rgba(255,255,255,0.2);">${ts}</div>
                                ${thumbBlock}
            ...
            // A5. VAULT LOCK CARD
            else if (originalMsg.startsWith('VAULT_LOCK_CARD::')) {
                try {
                    const d = JSON.parse(originalMsg.replace('VAULT_LOCK_CARD::', ''));
                    const typeLabel = d.type === 'instant' ? 'Self-locked' : 'Awaiting approval';
                    contentHtml = `
                    <div style="width:min(55%,260px);margin:0 auto;">
                        <div style="border-radius:14px;overflow:hidden;background:linear-gradient(170deg,#0e0406,#0d0404,#0a0303);border:1px solid rgba(139,0,0,0.4);box-shadow:0 12px 40px rgba(0,0,0,0.8);">
                            <div style="padding:16px 20px;text-align:center;">
            ...
            // WELCOME CARD (new member)
                ...
                try {
                    ...
                    contentHtml = `
                    <div style="width:min(60%,480px);min-width:240px;margin:0 auto;border-radius:16px;overflow:hidden;background:linear-gradient(170deg,#0c0a04 0%,#13100a 50%,#0c0a04 100%);border:1px solid rgba(197,160,89,0.6);box-shadow:0 12px 40px rgba(0,0,0,0.8),0 0 30px rgba(197,160,89,0.08);">
                        <div style="width:100%;padding:22px 0 16px;display:flex;flex-direction:column;align-items:center;background:radial-gradient(ellipse at center top,rgba(197,160,89,0.1) 0%,transparent 70%);">
                        ...
                        <div style="padding:4px 18px 22px;text-align:center;">
                            <div style="font-family:'Cinzel',serif;font-size:1.05rem;color:#fff;font-weight:700;letter-spacing:3px;text-transform:uppercase;margin-bottom:8px;">${DOMPurify.sanitize(d.name || '')}</div>
                    ...
                    contentHtml = `<div class="msg m-queen">New Member</div>`;
                }
            }
        ...
        if (originalMsg && (originalMsg.startsWith('WISHLIST::') || originalMsg.startsWith('TASK_FEEDBACK::') || originalMsg.startsWith('PROMOTION_CARD::') || originalMsg.startsWith('WELCOME_CARD::') || originalMsg.startsWith('ROUTINE_CHANGE::') || originalMsg.startsWith('TASK_REVIEW_CARD::') || originalMsg.startsWith('INVENTORY_CARD::') || originalMsg.startsWith('VAULT_UNLOCK_CARD::') || originalMsg.startsWith('VAULT_LOCK_CARD::') || originalMsg.startsWith('LEADERBOARD_REWARD_CARD::'))) {
            return `<div class="msg-row" style="justify-content:center; margin: 10px 0;"><div class="msg-col" style="align-items:center;">${contentHtml}<div class="msg-time">${timeStr}</div></div></div>`;
        }
