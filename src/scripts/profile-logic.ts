...
function renderChatMessage(msg: any, prevTs?: number): string {
    ...
    if (content.startsWith('RISKY_TRIBUTE_CARD::')) {
        try {
            ...
            const cardHtml = `
            <div style="width:min(90%,320px);margin:0 auto;border-radius:14px;overflow:hidden;background:linear-gradient(170deg,${rBg},#0a0a14);border:1px solid ${rBorderColor};box-shadow:0 8px 30px rgba(0,0,0,0.6);padding:14px 16px;">
                <div style="display:flex;align-items:center;gap:14px;">
                    <div style="flex-shrink:0;width:70px;display:flex;align-items:center;justify-content:center;">${rIconHtml}</div>
                    <div style="flex:1;min-width:0;">
                        <div style="font-family:'Orbitron',sans-serif;font-size:0.5rem;color:rgba(255,255,255,0.75);font-weight:600;margin-bottom:4px;">${d.senderName||''}</div>
                        <div style="font-family:'Orbitron',sans-serif;font-size:0.36rem;color:rgba(255,255,255,0.35);letter-spacing:2px;margin-bottom:3px;">RISKY SEND</div>
                        <div style="font-family:'Orbitron',sans-serif;font-size:0.45rem;color:${rResultColor};letter-spacing:1px;font-weight:700;margin-bottom:3px;">${d.cardName||''}</div>
                        <div style="font-family:'Rajdhani',sans-serif;font-size:0.65rem;color:rgba(255,255,255,0.45);">Staked ${(d.stakeAmount||0).toLocaleString()} · <span style="color:${rResultColor};font-weight:700;">${rResultText}</span></div>
                    </div>
                </div>
                <div style="margin-top:10px;text-align:center;"><button onclick="if(window.openStandaloneTribute)window.openStandaloneTribute('risky')" style="background:linear-gradient(135deg,rgba(197,160,89,0.15),rgba(197,160,89,0.05));border:1px solid rgba(197,160,89,0.4);color:#c5a059;font-family:'Orbitron',sans-serif;font-size:0.4rem;letter-spacing:2px;padding:6px 20px;border-radius:20px;cursor:pointer;">TRY YOUR LUCK</button></div>
            </div>`;
            return `<div class="cb-row" style="justify-content:center;padding:8px 0;">${cardHtml}${timeStr ? `<div class="chat-ts" style="text-align:center;margin-top:4px">${timeStr}</div>` : ''}</div>`;
        } catch (_) { /* fall through */ }
    }
