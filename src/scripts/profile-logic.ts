        ...
        container.innerHTML = data.posts.map((p: any) => {
            ...
            if (p.media_url) {
                if (locked) {
                    ...
                    mediaHTML = `<div style="position:relative;width:100%;overflow:hidden;background:#000;cursor:pointer;" onclick="var v=this.querySelector('video');if(v){this.querySelector('.vid-play-btn').style.display='none';v.style.pointerEvents='auto';v.setAttribute('controls','');v.setAttribute('src',v.dataset.src);v.load();v.play();}"><div class="vid-play-btn" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;z-index:2;"><div style="width:56px;height:56px;border-radius:50%;background:rgba(197,160,89,0.9);display:flex;align-items:center;justify-content:center;font-size:1.5rem;">▶</div></div><video data-src="${p.media_url}" playsinline${p.thumbnail_url ? ` poster="${p.thumbnail_url}" preload="none"` : ' preload="metadata"'} onerror="if(!this.dataset.retried){this.dataset.retried='1';this.src='/api/media?url='+encodeURIComponent(this.src);this.load();}" style="width:100%;display:block;max-height:340px;object-fit:cover;pointer-events:none;"></video></div>`;
                } else {
...
function _buildMobGlBubble(msg: any): string {
    ...
    const mediaHtml = msg.media_url
        ? (msg.media_type === 'video'
            ? `<div style="position:relative;width:100%;background:#000;border-radius:8px;margin-top:8px;cursor:pointer;" onclick="var v=this.querySelector('video');if(v){this.querySelector('.vid-play-btn').style.display='none';v.style.pointerEvents='auto';v.setAttribute('controls','');v.setAttribute('src',v.dataset.src);v.load();v.play();}"><div class="vid-play-btn" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;z-index:2;border-radius:8px;"><div style="width:50px;height:50px;border-radius:50%;background:rgba(197,160,89,0.9);display:flex;align-items:center;justify-content:center;font-size:1.3rem;">▶</div></div><video data-src="${msg.media_url}" playsinline preload="none" style="width:100%;border-radius:8px;max-height:260px;object-fit:cover;display:block;pointer-events:none;"></video></div>`
        ...
        const gridHTML = restPosts.length > 0 ? `
            ...
            const cardMediaHTML = !cardHasMedia ? `<div class="qk-card-img-placeholder">👑</div>` :
                locked
                    ...
                    : isVideo
                        ? `<div class="qk-card-img qk-card-media" onclick="window.openQkLightbox('video','${p.media_url}')"><video src="${p.media_url}" muted playsinline preload="metadata"${p.thumbnail_url ? ` poster="${p.thumbnail_url}"` : ''} class="qk-card-video"></video><div class="qk-play-icon">▶</div></div>`
...
export function openQkLightbox(type: 'image' | 'video', src: string) {
    ...
    // Programmatic play from user gesture — works on iOS unlike autoplay attribute
    if (type === 'video') {
