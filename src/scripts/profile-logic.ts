...
function _playQueenVideo(url: string) {
    ...
    // If overlay already open, just swap the video
    if (existing) {
    ...
    _queenVideosList.forEach((v: any) => {
        ...
        const thumb = v.thumbnail_url || '/queen-karin.png';
        circle.innerHTML = `<img src="${thumb}" style="width:100%;height:100%;object-fit:cover;" onerror="this.src='/queen-karin.png'" />
            <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.2);">
                <div style="width:0;height:0;border-style:solid;border-width:5px 0 5px 8px;border-color:transparent transparent transparent rgba(255,255,255,0.8);margin-left:1px;"></div>
            </div>`;
