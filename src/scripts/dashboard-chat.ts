    ...
    if (content.startsWith('TASK_REVIEW_CARD::')) {
        try {
            ...
            const taskTextLine = d.taskText
                ? `<div style="font-family:Rajdhani,sans-serif;font-size:0.78rem;color:rgba(255,255,255,0.5);letter-spacing:0.3px;margin-bottom:14px;line-height:1.5;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;">${purifier.sanitize(d.taskText)}</div>`
                : '';
            ...
            const commentLine = d.comment
                ? `<div style="margin-top:14px;padding:10px 14px;background:rgba(255,255,255,0.025);border-radius:8px;border:1px solid rgba(255,255,255,0.05);"><div style="font-family:Rajdhani,sans-serif;font-size:0.75rem;color:rgba(255,255,255,0.4);font-style:italic;line-height:1.5;">"${purifier.sanitize(d.comment)}"</div></div>`
                : '';
            return `
                <div class="chat-gift-wrap">
