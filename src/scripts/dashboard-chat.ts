...
function renderToHtml(m: any) {
    ...
    if (content.startsWith('TASK_REVIEW_CARD::')) {
        try {
            ...
            const taskTextLine = d.taskText
                ? `<div style="font-family:Rajdhani,sans-serif;font-size:0.72rem;color:rgba(255,255,255,0.4);line-height:1.4;overflow:hidden;display:-webkit-box;-webkit-line-clamp:1;-webkit-box-orient:vertical;">${purifier.sanitize(d.taskText)}</div>`
                : '';
            ...
            const commentLine = d.comment
                ? `<div style="font-family:Rajdhani,sans-serif;font-size:0.78rem;color:rgba(255,255,255,0.55);font-style:italic;margin-top:6px;line-height:1.4;">"${purifier.sanitize(d.comment)}"</div>`
                : '';
            return `
                <div class="chat-gift-wrap">
                    <div style="max-width:360px;width:85vw;border-radius:12px;overflow:hidden;background:#0c0c0c;border:1px solid ${accentBorder};">
                        ...
                        <div style="position:relative;display:flex;gap:14px;padding:12px 16px;">
                            <div style="position:absolute;top:10px;right:14px;font-family:Rajdhani,sans-serif;font-size:0.58rem;color:rgba(255,255,255,0.2);">${timeStr}</div>
                            ${thumbBlock}
    ...
    if (content.startsWith('LOCK_EXTENDED_CARD::')) {
        try {
            ...
            return `
                <div class="chat-gift-wrap">
                    <div style="max-width:260px;width:65vw;border-radius:14px;overflow:hidden;background:linear-gradient(170deg,#0e0406,#0d0404,#0a0303);border:1px solid rgba(139,0,0,0.5);box-shadow:0 12px 40px rgba(0,0,0,0.8);">
                        <div style="padding:16px 20px;text-align:center;">
