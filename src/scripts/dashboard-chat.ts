    ...
    if (content.startsWith('TASK_REVIEW_CARD::')) {
        try {
            ...
            const commentLine = d.comment
                ? `<div style="font-family:Rajdhani,sans-serif;font-size:0.68rem;color:rgba(255,255,255,0.35);font-style:italic;margin-top:6px;line-height:1.4;border-top:1px solid rgba(255,255,255,0.04);padding-top:6px;">"${purifier.sanitize(d.comment)}"</div>`
                : '';
            return `
                <div class="chat-gift-wrap">
