...
function _buildMobGlBubble(msg: any): string {
    ...
    const mediaHtml = msg.media_url
        ? (hasVideo
            ? `<div style="position:relative;width:100%;border-radius:10px;margin-top:8px;overflow:hidden;border:1px solid rgba(255,255,255,0.08);cursor:pointer;${_mobThumbStyle}min-height:200px;display:flex;align-items:center;justify-content:center;" onclick="var el=this,orig=el.innerHTML,origStyle=el.style.cssText,v=document.createElement('video');v.src='${msg.media_url.replace(/'/g, "\\'")}';v.controls=true;v.playsInline=true;v.autoplay=true;v.style.cssText='width:100%;max-height:280px;object-fit:cover;display:block;';v.onerror=function(){el.innerHTML=orig;el.style.cssText=origStyle;};el.innerHTML='';el.style.cursor='default';el.style.minHeight='auto';el.style.backgroundImage='none';el.onclick=null;el.appendChild(v);">${_mobPlaySvg}</div>`
