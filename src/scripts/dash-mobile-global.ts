...
function _buildMobGlBubble(msg: any): string {
    ...
    if (content.startsWith('RISKY_TRIBUTE_CARD::')) {
        try {
            ...
            return `<div style="display:flex;justify-content:center;padding:8px 0;margin-bottom:6px;"><div style="width:min(90%,320px);border-radius:14px;overflow:hidden;background:linear-gradient(170deg,${bg},#0a0a14);border:1px solid ${borderColor};box-shadow:0 8px 30px rgba(0,0,0,0.6);padding:14px 16px;"><div style="display:flex;align-items:center;gap:14px;"><div style="flex-shrink:0;width:70px;display:flex;align-items:center;justify-content:center;">${rIconHtml}</div><div style="flex:1;min-width:0;"><div style="font-family:'Orbitron',sans-serif;font-size:0.5rem;color:rgba(255,255,255,0.75);font-weight:600;margin-bottom:4px;">${d.senderName||''}</div><div style="font-family:'Orbitron',sans-serif;font-size:0.36rem;color:rgba(255,255,255,0.35);letter-spacing:2px;margin-bottom:3px;">RISKY SEND</div><div style="font-family:'Orbitron',sans-serif;font-size:0.45rem;color:${resultColor};letter-spacing:1px;font-weight:700;margin-bottom:3px;">${d.cardName||''}</div><div style="font-family:'Rajdhani',sans-serif;font-size:0.65rem;color:rgba(255,255,255,0.45);">Staked ${(d.stakeAmount||0).toLocaleString()} · <span style="color:${resultColor};font-weight:700;">${resultText}</span></div></div></div></div></div>`;
        } catch { /* fall through */ }
    }
    ...
    if (isQueen && hasPhoto) {
        ...
        return `<div style="margin-bottom:10px;overflow:hidden;"><div style="background:linear-gradient(170deg,#0e0b06 0%,#110d04 60%,#0a0703 100%);border:1.5px solid rgba(197,160,89,0.6);border-radius:14px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,0.6),0 0 20px rgba(197,160,89,0.08);"><div style="display:flex;align-items:center;gap:8px;padding:10px 12px;"><img src="/queen-nav.png" style="width:26px;height:26px;border-radius:50%;object-fit:cover;border:1.5px solid rgba(197,160,89,0.6);"><div style="flex:1;min-width:0;"><div style="display:flex;align-items:center;gap:3px;">${SVG_CROWN_MOB}<span style="font-family:'Cinzel',serif;font-size:0.5rem;color:#c5a059;letter-spacing:1px;font-weight:700;">QUEEN KARIN</span></div></div><span style="font-family:'Orbitron';font-size:0.32rem;color:rgba(197,160,89,0.45);">${time}</span></div><div style="width:100%;max-height:400px;overflow:hidden;cursor:pointer;" onclick="window._openGlobalLightbox&&window._openGlobalLightbox('${(msg.media_url || '').replace(/'/g, "\\'")}')"><img src="${msg.media_url}" style="width:100%;display:block;object-fit:cover;max-height:400px;" /></div><div style="padding:8px 12px 10px;"><div style="display:flex;align-items:center;gap:6px;margin-bottom:${captionText ? '5' : '0'}px;">${_likeBtn}${replyBtn}</div>${captionText ? `<div style="font-family:'Rajdhani',sans-serif;font-size:0.9rem;color:rgba(255,255,255,0.7);line-height:1.5;"><span style="font-family:'Cinzel',serif;font-size:0.48rem;color:#c5a059;font-weight:700;margin-right:5px;">QUEEN KARIN</span>${captionText}</div>` : ''}</div></div></div>`;
    }

    // QUEEN bubble (text or video)
    if (isQueen) {
        return `<div style="padding:8px 12px 10px;margin-bottom:6px;background:linear-gradient(135deg,rgba(197,160,89,0.14),rgba(100,75,15,0.08));border:1.5px solid rgba(197,160,89,0.75);border-radius:10px;box-shadow:0 0 14px rgba(197,160,89,0.12);overflow:hidden;"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:5px;gap:6px;"><div style="display:flex;align-items:center;gap:5px;flex-shrink:0;"><img src="/queen-nav.png" style="width:22px;height:22px;border-radius:50%;object-fit:cover;border:1.5px solid rgba(197,160,89,0.7);flex-shrink:0;"><div style="display:flex;align-items:center;gap:4px;white-space:nowrap;flex-shrink:0;">${SVG_CROWN_MOB}<span style="font-family:'Cinzel',serif;font-size:0.72rem;color:#c5a059;letter-spacing:1px;font-weight:700;white-space:nowrap;">QUEEN KARIN</span></div><span style="font-family:'Orbitron';font-size:0.38rem;color:rgba(197,160,89,0.6);white-space:nowrap;flex-shrink:0;"> \u00B7 ${time}</span></div><div style="display:flex;align-items:center;gap:4px;">${_likeBtn}${replyBtn}</div></div>${quoteHtml}<span style="font-family:'Rajdhani',sans-serif;font-size:0.95rem;color:rgba(255,255,255,0.7);line-height:1.5;">${content}</span>${mediaHtml}</div>`;
    }
...
async function _loadMobGlChallenges() {
    ...
    try {
        ...
        if (partRes?.ok) { const pj = await partRes.json(); (pj.participations || []).forEach((p: any) => { myStatuses[p.challenge_id] = p.status; }); }
        const now = Date.now();
        const active = (challenges || []).filter((c: any) => c.status === 'active');
        const upcoming = (challenges || []).filter((c: any) => c.status === 'draft' && c.start_date && new Date(c.start_date).getTime() > now);
        const all = [...active, ...upcoming];
        if (!all.length) { container.innerHTML = `<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:12px;padding:40px;"><span style="font-size:2rem;opacity:0.2;">\u2694</span><span style="font-family:'Orbitron';font-size:0.45rem;color:rgba(74,222,128,0.3);letter-spacing:2px;text-align:center;">NO ACTIVE CHALLENGES</span></div>`; return; }
        container.innerHTML = all.map((c: any) => {
            ...
            const stats = [{ label: 'Days', val: String(c.duration_days) }, { label: 'Tasks a day', val: String(c.tasks_per_day) }, { label: 'Window', val: `${c.window_minutes} min` }, { label: 'Still working', val: String(c.participant_active ?? '-') }, ...(daysLeft !== null && isLive ? [{ label: 'Days left', val: String(daysLeft) }] : []), ...(startsSoon && startDate ? [{ label: 'Starts', val: startDate }] : [])];
            const statsHtml = stats.map(({ label, val }) => `<div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:6px;"><span style="font-family:'Rajdhani',sans-serif;font-size:0.85rem;color:rgba(255,255,255,0.38);letter-spacing:0.5px;">${label}</span><span style="font-family:'Orbitron',sans-serif;font-size:0.85rem;color:rgba(197,160,89,0.9);font-weight:700;">${val}</span></div>`).join('');
            const badgeStyle = startsSoon ? 'background:rgba(251,191,36,0.9);color:#000;' : 'background:rgba(74,222,128,0.9);color:#000;';
            const badgeText = startsSoon ? 'SOON' : 'LIVE';
            const imgBlock = c.image_url ? `<img src="${c.image_url}" style="width:100%;height:100%;object-fit:cover;display:block;" onerror="this.style.display='none'" alt="${c.name}">` : `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:2rem;opacity:0.2;background:linear-gradient(135deg,rgba(197,160,89,0.1),rgba(197,160,89,0.04));">\u2605</div>`;
            return `<div style="margin:10px 12px;border-radius:16px;overflow:hidden;border:1px solid rgba(255,255,255,0.07);background:rgba(255,255,255,0.02);"><div style="display:flex;gap:0;"><div style="width:130px;min-height:180px;flex-shrink:0;position:relative;background:rgba(197,160,89,0.06);">${imgBlock}<div style="position:absolute;top:10px;left:10px;${badgeStyle}border-radius:6px;padding:3px 8px;font-family:'Orbitron',monospace;font-size:0.32rem;font-weight:700;letter-spacing:1px;">${badgeText}</div></div><div style="flex:1;padding:16px 16px 14px;display:flex;flex-direction:column;gap:10px;justify-content:space-between;min-width:0;"><div><div style="font-family:'Orbitron',sans-serif;font-size:0.95rem;color:#fff;font-weight:700;letter-spacing:1px;margin-bottom:5px;">${c.name}</div>${c.description ? `<div style="font-family:'Orbitron',sans-serif;font-size:0.58rem;color:rgba(255,255,255,0.35);line-height:1.5;letter-spacing:0.5px;">${c.description}</div>` : ''}</div><div style="display:flex;flex-direction:column;">${statsHtml}</div></div></div></div>`;
        }).join('');
