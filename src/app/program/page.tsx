...
const MOCK = {
    ...
    // detailed daily logs (mock — will come from program_daily + program_trials + program_spins)
    dailyLogs: [
        ...
        { day: 13, date: '2026-07-08', perfect: true, orders: [{ type: 'kneel', target: 8, done: 8 }, { type: 'spin', target: 1, done: 1 }, { type: 'trial', target: 1, done: 1 }, { type: 'tribute', target: 5, done: 5 }], trial: { prompt: 'Write a thank you to your basic_program.', response: 'Thank you for not giving up on me when I gave up on myself.' }, spin: { text: 'Queen grants 50 coins', type: 'reward' }, tribute: 5 },
...
const R = 'rgba(197,160,89,'; // gold accent base
...
const MECH_ICON: Record<string, { icon: string; label: string; desc?: string }> = {
    ...
    daily_check:   { icon: '\u25C8', label: 'Daily Check', desc: 'Submit photo proof that your device is locked and secure.' },
...
// No hardcoded fallback — orders come only from program_daily (the database)
...
// Convert a program_daily DB record into a DayLog for the day detail modal
function _toDayLog(rec: any): DayLog {
    ...
    return {
        ...
        orders,
        trial: null, // filled from program_trials if available
        spin: null,  // filled from program_spins if available
        tribute: 0,
    };
}
...
function _readCache() {
    ...
    try {
        const p = sessionStorage.getItem('_programProfileCache');
        const s = sessionStorage.getItem('_programSessionCache');
        const k = sessionStorage.getItem('_programKneelCache');
...
export default function ProgramPage() {
    ...
    const [programData, setProgramData] = useState<any>(_initCache.session?.active ? _initCache.session : (_liveProgramData?.active ? _liveProgramData : null)); // real DB data from /api/program/session
    ...
    const [dailyUploading, setChastityUploading] = useState(false);
    const [dailyStatus, setChastityStatus] = useState<'none' | 'pending' | 'approved' | 'rejected'>('none');
    const [dailyWindow, setChastityWindow] = useState<{ open: boolean; before: boolean; localHour: number; localMinute: number }>(() => {
        try {
    ...
    const [dailyPhotoUrl, setChastityPhotoUrl] = useState<string | null>(null);
    const [showChastityGate, setShowChastityGate] = useState(() => {
        // Force gate synchronously on first render — no waiting for effects/API
        // Condition: active program session + 6-10AM check window + proof not submitted
        try {
            const tz = Intl.DateTimeFormat().resolvedOptions().timeZone;
            ...
            const status = _initCache.session?.dailyCheck?.status;
    ...
    const [tab, setTab] = useState<'program' | 'chat' | 'queen' | 'global' | 'challenge'>('program');
    const tabRef = useRef<'program' | 'chat' | 'queen' | 'global' | 'challenge'>('program');
    tabRef.current = tab;
    ...
    const saveGambleResult = useCallback((data: Record<string, any>, orderType?: string) => {
        try {
            const existing = JSON.parse(localStorage.getItem('program_gamble_results') || '{}');
            localStorage.setItem('program_gamble_results', JSON.stringify({ ...existing, ...data }));
        } catch {}
        // Also persist to server so it survives hard refresh / cleared localStorage
        if (orderType) {
            const mid = (window as any).__programMemberId || '';
            if (mid) {
                fetch('/api/program/session', { method: 'POST', headers: { 'Content-Type': 'application/json' },
    ...
    const clearGambleResults = useCallback(() => {
        try { localStorage.removeItem('program_gamble_results'); } catch {}
    }, []);
    ...
    const [programSkipOpen, setProgramSkipOpen] = useState(false);
    ...
    const daysIn = programData?.daysIn ?? 0;
    const lockDays = programData?.session?.lock_days ?? 0;
    const dailyRecords = programData?.dailyRecords || [];
    const adjustments = programData?.adjustments || [];
    // Use programTasks (direct from program_member) as source of truth, fallback to program_daily orders
    const programTasks = programData?.programTasks;
    const rawOrders = programData?.today?.orders;
    const todayOrders = useMemo(() => {
        ...
        // Sync kneel order with kneelToday, and daily with dailyStatus
        return base.map((o: any) => {
            if (o.type === 'kneel') return { ...o, done: kneelToday };
            if (o.type === 'daily_check') return { ...o, done: dailyStatus === 'approved' ? o.target : 0, status: dailyStatus };
            return o;
        });
    }, [programTasks, rawOrders, kneelToday, dailyStatus, previewTasks]);
    const todayPerfectDb = programData?.today?.perfect ?? false;
    const todayPerfect = todayPerfectDb || (todayOrders.length > 0 && todayOrders.every((o: any) => (o.done || 0) >= (o.target || 1)));
    const todayRewardClaimed = programData?.today?.reward_claimed ?? false;
    ...
    useEffect(() => {
        try {
            const savedFU = localStorage.getItem('program_followup');
            if (savedFU) { setFollowUp(JSON.parse(savedFU)); }
            const savedGamble = localStorage.getItem('program_gamble_results');
            if (savedGamble) {
    ...
    useEffect(() => {
        ...
        const orders = programData?.programTasks || (programData?.today?.orders ? (typeof programData.today.orders === 'string' ? JSON.parse(programData.today.orders) : programData.today.orders) : []);
    ...
    useEffect(() => {
        try {
            if (followUp) localStorage.setItem('program_followup', JSON.stringify(followUp));
            else localStorage.removeItem('program_followup');
        } catch {}
    }, [followUp]);
    ...
    useEffect(() => {
        try {
            const stored = localStorage.getItem('program_cooldowns');
            if (stored) {
    ...
    useEffect(() => {
        try {
            localStorage.setItem('program_cooldowns', JSON.stringify({
    ...
    useEffect(() => {
        if (mechOverlay?.order?.type !== 'simon_says') {
            if (simonWaitRef.current) { clearInterval(simonWaitRef.current); simonWaitRef.current = null; }
                ...
                if (s.phase && s.phase !== 'idle') {
                    ...
                    if (s.phase === 'waiting' && s.waitUntil) {
                        if (Date.now() >= s.waitUntil) {
                            // Time already passed — pick task immediately
                            ...
                            setSimonWaitUntil(s.waitUntil);
                            setSimonPhase('waiting');
                        }
                    } else if (s.phase === 'task' && s.currentTask) {
                        setSimonCurrentTask(s.currentTask);
                        setSimonTaskSecs(s.currentTask.timeLimit || 30);
                        setSimonTaskLimit(s.currentTask.timeLimit || 30);
                        setSimonPhase('task');
        ...
        setSimonStep(0);
        setSimonProofs([]);
        setSimonCurrentTask(null);
        setSimonLastTask(null);
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    useEffect(() => {
        ...
        const chain: any[] = (mechOverlay as any)?.order?.config?.chainTasks || [];
        ...
        simonWaitRef.current = setInterval(() => {
            if (Date.now() >= simonWaitUntil) {
                ...
                const task = src[Math.floor(Math.random() * src.length)];
                setSimonCurrentTask(task);
                setSimonLastTask(task);
                setSimonTaskSecs(task.timeLimit || 30);
    ...
    useEffect(() => {
        if (!programData?.session?.started_at) return;
        const lockStart = programData.session.started_at;
        const lockEnd = programData.session.expires_at || new Date(Date.now() + 86400000).toISOString();
        // Update immediately when programData loads (so timer isn't 00:00:00 on mount)
    ...
    // Update daily window every minute — pure local time math, no server call
    useEffect(() => {
        const update = () => {
            try {
                const tz = Intl.DateTimeFormat().resolvedOptions().timeZone;
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined' && new URLSearchParams(window.location.search).has('preview')) {
            ...
            setProgramData({ active: true, daysIn: 7, programTasks: previewTask, today: { perfect: false, orders: [], trial_prompt: 'Write about your devotion to Queen Karin. 200 words minimum.' }, submissions: [], adjustments: [], session: { lock_days: 30, id: 'preview', started_at: new Date(Date.now() - 7 * 86400000).toISOString(), expires_at: new Date(Date.now() + 23 * 86400000).toISOString() }, dailyRecords: [] });
            setLoading(false);
            setTab('challenge');
            return;
        }
        ...
        try { sessionStorage.removeItem('_programProfileCache'); sessionStorage.removeItem('_programSessionCache'); sessionStorage.removeItem('_programKneelCache'); } catch {}
        ...
        profileReady
            .then(async (rawData: any) => {
                ...
                console.log('[PROGRAM] Loaded profile:', data);
                ...
                if (data.memberId || user?.id) {
                    ...
                    (window as any)._programHeartbeat = hb;
                    ...
                    try {
                        ...
                        if (pEmail) {
                            ...
                            (window as any)._programPresenceCh = presenceCh;
                        }
                    } catch (_) {}
                }
                ...
                (window as any).__programMemberId = memberId;
                ...
                // Fire and forget — same as /profile page, don't block program load on chat history
                initChatSystem().catch(() => {});

                // Load program session — use cache from /profile splash if available
                const sessionReady = _cachedSession
                    ? Promise.resolve(_cachedSession)
                    : fetch(`/api/program/session?memberId=${encodeURIComponent(memberId)}&tz=${encodeURIComponent(Intl.DateTimeFormat().resolvedOptions().timeZone)}`).then(r => r.json());

                sessionReady.then(async vd => {
                        console.log('[PROGRAM] Session data:', vd);

                        // Auto-enroll: if no active session, create one automatically
                        if (!vd.active) {
                            console.log('[PROGRAM] No active session — auto-enrolling...');
                            try {
                                const enrollRes = await fetch('/api/program/session', {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({ action: 'auto_enroll', memberId, tz: Intl.DateTimeFormat().resolvedOptions().timeZone }),
                                });
                                const enrollData = await enrollRes.json();
                                if (enrollData.success) {
                                    // Re-fetch the full session data
                                    const freshRes = await fetch(`/api/program/session?memberId=${encodeURIComponent(memberId)}&tz=${encodeURIComponent(Intl.DateTimeFormat().resolvedOptions().timeZone)}`);
                                    const freshVd = await freshRes.json();
                                    if (freshVd.active) {
                                        vd = freshVd;
                                    }
                                }
                            } catch (e) {
                                console.error('[PROGRAM] Auto-enroll failed:', e);
                            }
                        }

                        if (vd.active) {
                            setProgramData(vd);
                            _liveProgramData = vd; // persist across remounts
                            ...
                            // Check daily check status for today
                            {
                                const todayOrd = vd.today?.orders ? (typeof vd.today.orders === 'string' ? JSON.parse(vd.today.orders) : vd.today.orders) : [];
                                const cc = todayOrd.find((o: any) => o.type === 'daily_check');
                                // Read daily status from program_check_log table (source of truth)
                                const chk = vd.dailyCheck;
                                if (chk?.status === 'approved') setChastityStatus('approved');
                                    ...
                                    // Fallback: check orders JSON for legacy approved daily (before program_check_log existed)
                                    const todayOrd = vd.today?.orders ? (typeof vd.today.orders === 'string' ? JSON.parse(vd.today.orders) : vd.today.orders) : [];
                                    const cc = todayOrd.find((o: any) => o.type === 'daily_check');
                            ...
                            if (vd.dailyWindow && !_cachedSession) setChastityWindow(vd.dailyWindow);
                            if (_cachedKneel) {
                            ...
                            fetch('/api/program/session', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'ensure_today', memberId, tz: Intl.DateTimeFormat().resolvedOptions().timeZone }) })
                                .then(r => r.json())
                                .then(ensureRes => {
                                    if (ensureRes.ended) {
                                        // Chastity check failed — program terminated
                                        try { localStorage.removeItem('program_cooldowns'); } catch {}
                                        setReleaseOverlay({ reason: 'Chastity check not submitted or approved. Program terminated.' });
                                        return;
                                    }
                                    return fetch(`/api/program/session?memberId=${encodeURIComponent(memberId)}&tz=${encodeURIComponent(Intl.DateTimeFormat().resolvedOptions().timeZone)}`)
                                        .then(r2 => r2.json())
                                        .then(vd2 => {
                                            if (vd2.active) {
                                                setProgramData(vd2);
                                                if (vd2.dailyWindow) setChastityWindow(vd2.dailyWindow);
                                                // Re-read daily status from fresh data
                                                const chk2 = vd2.dailyCheck;
                            ...
                            // Listen on BOTH program_sessions AND profiles for maximum reliability
                            const rtSub = supabase
                                .channel('program_release_watch')
                                .on('postgres_changes', {
                                    event: 'UPDATE',
                                    schema: 'public',
                                ...
                                .subscribe();
                            (window as any)._programRtSub = rtSub;

                            // Realtime: listen for Queen's review actions (broadcast from API)
                            if (_cachedProfile?.ID) {
                                const notifySub = supabase
                                    .channel(`program-notify-${_cachedProfile.ID}`)
                                    .on('broadcast', { event: 'daily_reviewed' }, (msg: any) => {
                                        ...
                                        if (mid) {
                                            fetch(`/api/program/session?memberId=${encodeURIComponent(mid)}`).then(r => r.json()).then(vd2 => {
                                                if (vd2.active) setProgramData(vd2);
                                            }).catch(() => {});
                                        }
                                    })
                                    ...
                                    .subscribe();
                                (window as any)._programDailySub = notifySub;
                            }
                        }
                    ...
                    .catch(e => {
                        console.error('[PROGRAM] Session fetch failed:', e);
                        setLoading(false);
                    });
            })
        ...
        import('@/scripts/profile-logic').then(({ loadTributes, buyRealCoins }) => {
            loadTributes();
            // Program mode: wrap render to filter out crowdfund items (gifts only)
            const origRender = (window as any)._renderTributeGridMobile;
            if (origRender) {
                (window as any)._renderTributeGridMobile = (grid: HTMLElement) => {
                    (window as any)._programHideCrowdfund = true;
                    origRender(grid);
                    delete (window as any)._programHideCrowdfund;
                };
            }
            // Program boost wallet — show coin packages inline instead of navigating to exchequer
            (window as any)._showBoostWallet = () => {
                const existing = document.getElementById('_programBoostOverlay');
                if (existing) existing.remove();
                const R = 'rgba(197,160,89,';
                const pkgs = [
                ...
                overlay.id = '_programBoostOverlay';
                ...
                box.innerHTML = `
                    ...
                    <div style="width:100%;display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:4px;">
                        ${pkgs.map(p => `
                            <button class="_programBoostPkg" data-coins="${p.coins}" style="background:${R}0.04);border:1px solid ${R}0.2);border-radius:10px;padding:14px 8px;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:4px;transition:all 0.2s;">
                    ...
                    <button id="_programBoostCancel" style="background:none;border:none;color:rgba(255,255,255,0.15);font-family:Rajdhani,sans-serif;font-size:0.6rem;letter-spacing:3px;padding:8px 20px;cursor:pointer;margin-top:4px;">CANCEL</button>
                `;
                overlay.appendChild(box);
                document.body.appendChild(overlay);
                box.querySelector('#_programBoostCancel')!.addEventListener('click', () => overlay.remove());
                box.querySelectorAll('._programBoostPkg').forEach(btn => {
        ...
        return () => {
            ...
            if ((window as any)._programHeartbeat) { clearInterval((window as any)._programHeartbeat); (window as any)._programHeartbeat = null; }
            try { if ((window as any)._programRtSub) { createClient().removeChannel((window as any)._programRtSub); (window as any)._programRtSub = null; } } catch {}
            try { if ((window as any)._programDailySub) { createClient().removeChannel((window as any)._programDailySub); (window as any)._programDailySub = null; } } catch {}
            try { if ((window as any)._programPresenceCh) { createClient().removeChannel((window as any)._programPresenceCh); (window as any)._programPresenceCh = null; } } catch {}
        };
    }, []);
    ...
    useEffect(() => {
        ...
        const iv = setInterval(() => {
            if (Date.now() >= chatExpiresAt) {
                ...
                // Only navigate to program if user is ON the chat tab — don't kick from WORK/other tabs
                if (tabRef.current === 'chat') setTab('program');
            }
        }, 1000);
        return () => clearInterval(iv);
    }, [chatExpiresAt]);

    // Auto-force daily gate once programData loads (catches case where cache was empty)
    const dailyGateAutoOpened = useRef(false);
    useEffect(() => {
        if (dailyGateAutoOpened.current) return;
        if (!dailyWindow.open) return;
        if (dailyStatus === 'approved' || dailyStatus === 'pending') return;
        ...
        // Require active program session — daysIn 0+ is fine (Day 1 needs morning check too)
        if (!programData?.session?.id) return;
        dailyGateAutoOpened.current = true;
        setShowChastityGate(true);
    }, [dailyWindow, programData, dailyStatus]);
    ...
    useEffect(() => {
        ...
        if (dailyStatus === 'pending' || dailyStatus === 'approved') {
            setShowChastityGate(false);
        }
    }, [dailyStatus, gateSuccess]);
    ...
    const sendAttentionCard = useCallback((task: typeof ATTENTION_TASKS[0], opts: { completed?: boolean; skipped?: boolean; result?: string } = {}) => {
        ...
        }).catch(e => console.error('[program] attention card error:', e));
    }, [profile]);

    // Build program context string for Vlad
    const buildProgramContext = useCallback(() => {
    ...
    const sendVladMsg = useCallback(async (msg: string, isAuto = false) => {
        ...
        try {
            const res = await fetch('/api/ai-chat', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ message: msg, memberId, programContext: buildProgramContext() }),
            });
    ...
    }, [vladSending, profile, buildProgramContext, vladOpen]);
    ...
    // Vlad welcome on first program arrival (after video submission)
    useEffect(() => {
        if (loading || !profile) return;
        try {
            const first = sessionStorage.getItem('_programFirstArrival');
            if (first) {
                sessionStorage.removeItem('_programFirstArrival');
                const name = profile?.name || '';
                setTimeout(() => {
                    vladReact(`${name} has just been locked in the program for the first time. Welcome them to their cage. Be dark, theatrical, slightly menacing but welcoming. This is their first moment inside.`);
                }, 1500);
            }
        } catch {}
    ...
    // Program-specific post-kneel callback — updates React state + program daily order
    useEffect(() => {
        (window as any).__programKneelCallback = (data: any) => {
            if (typeof data?.todayKneeling === 'number') setKneelToday(data.todayKneeling);
            const mid = profile?.member_id || profile?.memberId || '';
            if (programData?.session?.id && mid) {
                fetch('/api/program/session', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
        ...
        return () => { delete (window as any).__programKneelCallback; };
    }, [profile, programData, todayOrders, vladReact]);
    ...
    const spin = useCallback(() => {
        ...
        setTimeout(() => {
            ...
            // Record spin in DB
            if (programData?.session?.id) {
                fetch('/api/program/session', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'spin', memberId: profile?.member_id || profile?.memberId || '', resultText: WHEEL[idx].text, resultType: WHEEL[idx].type }) }).catch(() => {});
            }
        }, 4000);
    ...
    // Cancel chat gate task — show shame, then 5h lockout + back to program
    ...
    if (releaseOverlay) return (
        <div style={{ position: 'fixed', inset: 0, zIndex: 2147483647, background: 'linear-gradient(180deg, #050508 0%, #0a0008 50%, #050508 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32 }}>
            <div style={{ width: 48, height: 1, background: 'rgba(197,160,89,0.4)', marginBottom: 32 }} />
            <div style={{ fontFamily: "'Cinzel', serif", fontSize: '1.1rem', color: 'rgba(197,160,89,0.9)', letterSpacing: 6, fontWeight: 700, textAlign: 'center', marginBottom: 16 }}>LOCK RELEASED</div>
            ...
            <div style={{ width: 48, height: 1, background: 'rgba(197,160,89,0.2)', marginBottom: 40 }} />
            <button onClick={() => { window.location.href = '/profile'; }} style={{ padding: '14px 40px', background: 'none', border: '1px solid rgba(197,160,89,0.4)', borderRadius: 6, color: 'rgba(197,160,89,0.9)', fontFamily: "'Cinzel', serif", fontSize: '0.55rem', letterSpacing: 4, cursor: 'pointer', textTransform: 'uppercase', transition: 'all 0.3s' }}>
    ...
    const setPreviewMech = useCallback((type: string) => {
        ...
        setTaskText(''); setTaskSubmitted({}); setProgramSkipOpen(false);
        setTrialOpen(false); setTrialText(''); setTrialDone(false);
        try { localStorage.removeItem('program_followup'); localStorage.removeItem('program_gamble_results'); localStorage.removeItem('ss_state'); } catch {}
    }, []);
    ...
    return (
        <div style={{ background: '#080810', minHeight: '100vh', maxWidth: 480, margin: '0 auto', position: 'relative', overflowX: 'clip' }}>
            ...
            {/* ══════════════════════════════════════════════
                PROGRAM TAB — main scroll
            ══════════════════════════════════════════════ */}
            <div style={{ display: tab === 'program' ? 'flex' : 'none', flexDirection: 'column', alignItems: 'center', paddingBottom: 100, position: 'relative', zIndex: 1, minHeight: '100vh' }}>

                {/* ── HALO HERO SECTION (profile style — inline for desktop compat) ── */}
                <div style={{ width: '100%', position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 20px 28px', boxSizing: 'border-box' }}>
                    {/* Circle */}
                    <div style={{
                        position: 'relative', zIndex: 2, width: 340, height: 340, borderRadius: '50%',
                        ...
                        <div style={{ fontFamily: "'Cinzel', serif", fontSize: '1.8rem', color: '#fff', textTransform: 'uppercase', letterSpacing: 2, textShadow: '0 0 20px rgba(255,255,255,0.4)', lineHeight: 1, textAlign: 'center', maxWidth: '90%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: 400, marginBottom: 5 }}>
                            {profile?.name || ''}
                        </div>
                        <div style={{ fontFamily: "'Orbitron', sans-serif", fontSize: '0.7rem', color: '#c5a059', letterSpacing: 4, textTransform: 'uppercase', fontWeight: 400 }}>
                            {profile?.hierarchy || 'Hall Boy'}
                        </div>
                        {programData?.active && (
                            <div style={{ fontFamily: "'Orbitron', sans-serif", fontSize: '0.5rem', color: 'rgba(197,160,89,0.6)', letterSpacing: 3, marginTop: 8 }}>
                                DAY {daysIn + 1} OF {lockDays}
                            </div>
                        )}
                        <div style={{ fontFamily: "'Orbitron', sans-serif", fontSize: '0.5rem', color: '#666', letterSpacing: 2, marginTop: 15, marginBottom: 5 }}>DAILY PROGRESS</div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(8, 1fr)', gap: 4, width: '60%', marginBottom: 10 }}>
                            {Array.from({ length: 8 }).map((_, i) => (
                                <div key={i} style={{
                                    height: 6, borderRadius: 1,
                                    background: i < (programData?.todayKneeling || 0) ? '#c5a059' : 'rgba(255,255,255,0.12)',
                                    border: `1px solid ${i < (programData?.todayKneeling || 0) ? '#f0d080' : 'rgba(255,255,255,0.15)'}`,
                                    boxShadow: i < (programData?.todayKneeling || 0) ? '0 0 10px rgba(197,160,89,0.5)' : 'none',
                    ...
                    {/* Stats pill */}
                    <div style={{
                        position: 'relative', zIndex: 3, marginTop: -65, width: '94%',
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, width: '45%' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <span id="programMerit" style={{ fontFamily: "'Orbitron', sans-serif", fontSize: 'clamp(1.2rem, 5vw, 1.5rem)', color: '#fff', fontWeight: 800, lineHeight: 1 }}>{profile?.score ?? 0}</span>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, width: '45%' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <span id="programCoins" style={{ fontFamily: "'Orbitron', sans-serif", fontSize: 'clamp(1.2rem, 5vw, 1.5rem)', color: '#fff', fontWeight: 800, lineHeight: 1 }}>{profile?.wallet ?? 0}</span>
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#c5a059" strokeWidth="1.5" style={{ opacity: 0.7 }}><circle cx="12" cy="12" r="10" /><path d="M12 6v12M8 10h8M8 14h8" /></svg>
                            </div>
                ...
                {/* ── PROGRAM STATUS CARD ── */}
                {programData?.active && (
                    <div style={{ width: '100%', padding: '0 16px', marginTop: 8, marginBottom: 8 }}>
                        <div style={{
                            width: '100%', borderRadius: 14, padding: '20px 16px',
                            ...
                            <div style={{ textAlign: 'center' }}>
                                <div style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '1.4rem', color: '#c5a059', fontWeight: 800, textShadow: '0 0 15px rgba(197,160,89,0.3)' }}>{daysIn + 1}</div>
                                <div style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '0.45rem', color: 'rgba(255,255,255,0.3)', letterSpacing: 2, marginTop: 4 }}>DAY</div>
                            </div>
                            <div style={{ width: 1, height: 40, background: 'rgba(197,160,89,0.15)' }} />
                            <div style={{ textAlign: 'center' }}>
                                <div style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '1.4rem', color: '#c5a059', fontWeight: 800, textShadow: '0 0 15px rgba(197,160,89,0.3)' }}>{programData?.session?.current_streak || 0}</div>
                                <div style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '0.45rem', color: 'rgba(255,255,255,0.3)', letterSpacing: 2, marginTop: 4 }}>STREAK</div>
                            </div>
                            <div style={{ width: 1, height: 40, background: 'rgba(197,160,89,0.15)' }} />
                            <div style={{ textAlign: 'center' }}>
                                <div style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '1.4rem', color: '#c5a059', fontWeight: 800, textShadow: '0 0 15px rgba(197,160,89,0.3)' }}>{programData?.session?.total_perfect_days || 0}</div>
                                <div style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '0.45rem', color: 'rgba(255,255,255,0.3)', letterSpacing: 2, marginTop: 4 }}>PERFECT</div>
                            </div>
                            <div style={{ width: 1, height: 40, background: 'rgba(197,160,89,0.15)' }} />
                            <div style={{ textAlign: 'center' }}>
                                <div style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '1.4rem', color: '#fff', fontWeight: 800 }}>{remaining.d}</div>
                                <div style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '0.45rem', color: 'rgba(255,255,255,0.3)', letterSpacing: 2, marginTop: 4 }}>LEFT</div>
                            </div>
                        </div>
                    </div>
                )}
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 8, padding: '6px 0 0' }}>
                    {Array.from({ length: todayOrders.find((o: any) => o.type === 'kneel')?.target || 4 }).map((_, i) => (
                        <div key={i} style={{
                            ...
                            background: i < kneelToday ? '#c5a059' : 'rgba(255,255,255,0.08)',
                            border: `1px solid ${i < kneelToday ? 'rgba(197,160,89,0.7)' : 'rgba(255,255,255,0.15)'}`,
                            boxShadow: i < kneelToday ? '0 0 6px rgba(197,160,89,0.3)' : 'none',
                            transition: 'all 0.3s ease',
                        }} />
                    ))}
                </div>
                ...
                {cancelShame && (
                    <div style={{
                        ...
                        <div style={{ textAlign: 'center', maxWidth: 320 }}>
                            ...
                            <div style={{ fontFamily: 'Cinzel, serif', fontSize: '1.1rem', color: '#c5a059', letterSpacing: '3px', textShadow: `0 0 30px ${R}0.4)`, marginBottom: 16, lineHeight: 1.5 }}>
                                PATHETIC
                            </div>
                ...
                <div style={{ width: '100%', padding: '0 16px 36px', marginTop: 20 }}>
                    {todayPerfect ? (
                        ...
                        <div style={{
                            ...
                            {todayOrders.filter((o: any) => o.type !== 'daily_check' && o.type !== 'kneel').length === 0 && (
                                <div style={{ padding: '20px', textAlign: 'center', fontFamily: 'Rajdhani, sans-serif', fontSize: '0.85rem', color: 'rgba(255,255,255,0.2)', letterSpacing: '2px' }}>
                                    {programData?.today ? 'NO ADDITIONAL TASKS' : 'LOADING...'}
                                </div>
                            )}
                            {todayOrders.filter((o: any) => o.type !== 'daily_check' && o.type !== 'kneel').map((o: any, i: number, arr: any[]) => {
                                const completed = o.done >= o.target;
                                const subs = programData?.submissions || [];
                        ...
                        <div style={{ textAlign: 'center', marginTop: 12, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
                            ...
                            {todayRewardClaimed ? (
                                ...
                                <button onClick={() => {
                                    ...
                                    if (programData?.session?.id) {
                                        fetch('/api/program/session', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'claim_reward', memberId: profile?.member_id || profile?.memberId || '' }) }).catch(() => {});
                                    }
                                }} style={{
                ...
                <div style={{ width: '100%', display: 'flex', justifyContent: 'center', padding: '0 20px 32px' }}>
                    <button onClick={() => (window as any).openStandaloneTribute?.('wishlist')} style={{
                        ...
                        <span style={{ fontFamily: 'Cinzel, serif', fontSize: '0.85rem', color: '#c5a059', letterSpacing: 3, fontWeight: 700 }}>TRIBUTE</span>
                    </button>
                </div>
                ...
                <div style={{ textAlign: 'center', padding: '20px 0 40px' }}>
                    ...
                    <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: '0.85rem', color: 'rgba(255,255,255,0.45)', letterSpacing: '2px', marginTop: 8 }}>DENIED {programData?.begs?.filter((b: any) => b.status === 'denied').length ?? 0} TIMES</div>
                </div>
            </div>
            ...
            <div style={{ display: tab === 'queen' ? 'flex' : 'none', flexDirection: 'column', position: 'fixed', inset: 0, zIndex: 40, background: '#050508' }}>
                ...
                <div style={{ padding: '16px 20px', borderBottom: `1px solid ${R}0.1)`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button onClick={() => setTab('program')} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.45)', fontSize: '1.2rem', cursor: 'pointer' }}>&#10005;</button>
                </div>
                ...
                <div style={{
                    ...
                    {[
                        ...
                        { v: programData?.begs?.filter((b: any) => b.status === 'denied').length ?? 0, l: 'DENIED' },
                        { v: programData?.session?.current_streak ?? 0, l: 'STREAK' },
                        { v: programData?.trials?.length ?? 0, l: 'TRIALS' },
                    ].map(s => (
                ...
                <div style={{ width: '100%', padding: '0 16px 28px' }}>
                    ...
                    {(() => {
                        const lockStart = new Date(programData?.session?.started_at || new Date().toISOString());
                        ...
                        return (
                            <div style={{ background: `${R}0.04)`, border: `1px solid ${R}0.12)`, borderRadius: 12, padding: '16px 12px 12px' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'center', gap: 20, marginTop: 12, paddingTop: 10, borderTop: '1px solid rgba(197,160,89,0.1)' }}>
                                    {[
                    ...
                    <div key={t} style={{ display: tab === t ? 'flex' : 'none', flexDirection: 'column', position: 'fixed', top: (previewMode && t === 'challenge') ? 36 : 0, left: 0, right: 0, bottom: 0, zIndex: 40, background: t === 'challenge' ? 'linear-gradient(rgba(4,3,10,0.78) 0%, rgba(4,3,10,0.85) 100%), url(/work-bg.jpg) center top / cover no-repeat' : '#050508' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    ...
                                    <div>
                                        ...
                                        <div style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '0.75rem', color: `${R}0.65)`, letterSpacing: '2px' }}>
                                            {chatExpiresAt ? (() => { const left = Math.max(0, Math.ceil((chatExpiresAt - Date.now()) / 1000)); const m = Math.floor(left / 60); const s = left % 60; return `${m}:${s.toString().padStart(2, '0')} LEFT`; })() : 'BASIC PROGRAM'}
                                        </div>
                                    </div>
                                </div>
                            ) : t === 'challenge' ? (
                        ...
                        {isLocked && t === 'chat' ? (
                            ...
                            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
                                {(() => {
                                    const tasks = todayOrders.filter((o: any) => o.type !== 'daily_check' && o.type !== 'kneel');
                                    const subs = programData?.submissions || [];
                                    ...
                                    return (
                                        <>
                                            {/* ── DAY HERO + TASK INDEX (side by side) ── */}
                                            <div style={{ padding: '24px 20px 20px', borderBottom: '1px solid rgba(197,160,89,0.1)' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                                                    {/* Left: day number */}
                                                    <div style={{ flexShrink: 0 }}>
                                                        <div style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '0.45rem', color: 'rgba(197,160,89,0.55)', letterSpacing: '6px', marginBottom: 2 }}>DAY</div>
                                                        <div style={{ fontFamily: 'Cinzel, serif', fontSize: '4.2rem', color: 'rgba(255,255,255,0.95)', fontWeight: 700, lineHeight: 0.85, letterSpacing: '-2px' }}>{daysIn + 1}</div>
                                                    </div>

                                                    {/* Right: task circles */}
                                                    {tasks.length > 0 && (
                                                        <div style={{ flex: 1, display: 'flex', flexDirection: 'row', gap: 6, overflowX: 'auto', scrollbarWidth: 'none', paddingBottom: 2 }}>
                                                            {tasks.map((o: any, i: number) => {
                                                                ...
                                                                const circleColor = isDone ? 'rgba(80,200,120,0.8)' : pending ? 'rgba(197,160,89,0.7)' : isCurrent ? 'rgba(197,160,89,0.95)' : 'rgba(255,255,255,0.15)';
                                                                const circleBorder = isDone ? 'rgba(80,200,120,0.4)' : pending ? 'rgba(197,160,89,0.35)' : isCurrent ? 'rgba(197,160,89,0.6)' : 'rgba(255,255,255,0.1)';
                                                                const circleBg = isDone ? 'rgba(80,200,120,0.09)' : pending ? 'rgba(197,160,89,0.08)' : isCurrent ? 'rgba(197,160,89,0.12)' : 'rgba(255,255,255,0.03)';

                                                                return (
                                                                    <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, flex: '0 0 56px' }}>
                                                                        {/* Number */}
                                                                        <div style={{ fontFamily: 'Orbitron, monospace', fontSize: '0.38rem', color: isDone ? 'rgba(80,200,120,0.5)' : isCurrent ? 'rgba(197,160,89,0.6)' : 'rgba(255,255,255,0.2)', letterSpacing: '1px' }}>
                                                                            {String(i + 1).padStart(2, '0')}
                                                                        </div>
                                                                        {/* Status circle */}
                                                                        <div style={{ width: 40, height: 40, borderRadius: '50%', border: `1.5px solid ${circleBorder}`, background: circleBg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: isCurrent ? '0 0 12px rgba(197,160,89,0.3)' : 'none', animation: isCurrent ? 'vPulse 2s ease infinite' : 'none' }}>
                                                                            {isDone && <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke={circleColor} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>}
                                                                            {pending && !isDone && <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke={circleColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9" /><polyline points="12 7 12 12 15 15" /></svg>}
                                                                            {isCurrent && !isDone && !pending && <svg viewBox="0 0 24 24" width="9" height="9" fill={circleColor}><circle cx="12" cy="12" r="6" /></svg>}
                                                                            {!isDone && !pending && !isCurrent && <svg viewBox="0 0 24 24" width="7" height="7" fill="rgba(255,255,255,0.15)"><circle cx="12" cy="12" r="5" /></svg>}
                                                                        </div>
                                                                        {/* Task name */}
                                                                        <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: '0.58rem', letterSpacing: '0px', color: isDone ? 'rgba(255,255,255,0.2)' : isCurrent ? 'rgba(255,255,255,0.9)' : pending ? 'rgba(197,160,89,0.6)' : 'rgba(255,255,255,0.3)', textAlign: 'center', lineHeight: 1.2, wordBreak: 'break-word' }}>
                                                                            {taskLabel}
                                                                        </div>
                                                                    </div>
                                                                );
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 16 }}>
                                                    ...
                                                    <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: '0.65rem', letterSpacing: '4px', color: allDone && pendingCount === 0 ? 'rgba(80,200,120,0.65)' : 'rgba(255,255,255,0.25)' }}>
                                                    ...
                                                    <div style={{ margin: '20px 16px 120px', animation: 'vFadeIn 0.3s ease' }}>
                                                        ...
                                                        <div style={{ padding: '22px 20px 24px', background: 'rgba(15,12,25,0.6)' }}>
                                                            ...
                                                            {o.type === 'trial' && (
                                                                <div>
                                                                    <div style={{ fontFamily: 'Cinzel, serif', fontSize: '0.95rem', color: 'rgba(255,255,255,0.5)', lineHeight: 1.8, marginBottom: 16 }}>
                                                                        {programData?.today?.trial_prompt || 'No trial assigned yet.'}
                                                                    </div>
                                                                    {!trialDone && !trialOpen && (
                                                                        ...
                                                                        <>
                                                                            ...
                                                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 10 }}>
                                                                                ...
                                                                                <button onClick={() => {
                                                                                    ...
                                                                                    if (programData?.session?.id) {
                                                                                        fetch('/api/program/session', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'trial', memberId: profile?.member_id || profile?.memberId || '', prompt: programData?.today?.trial_prompt || 'Daily trial', response: trialText }) }).catch(() => {});
                                                                                    }
                                                            ...
                                                            {!['spin','trial','tribute','silence'].includes(o.type) && (() => {
                                                                ...
                                                                const existingSub = (programData?.submissions || []).find((s: any) => s.order_type === o.type);
                                                                ...
                                                                const submitTask = async (opts: { text?: string; photoUrl?: string }) => {
                                                                    console.log('[program] submitTask called:', o.type, 'mid:', mid);
                                                                    setTaskSubmitted(p => ({ ...p, [o.type]: true }));
                                                                    if (previewMode) { setMechOverlay(null); return; }
                                                                    try {
                                                                        const resp = await fetch('/api/program/session', { method: 'POST', headers: { 'Content-Type': 'application/json' },
                                                                        ...
                                                                        console.log('[program] submit_task response:', resp.status, result);
                                                                        if (!resp.ok) alert('Submit failed: ' + (result.error || 'unknown error'));
                                                                        setTaskText('');
                                                                        // Refresh program data to get updated submissions
                                                                        if (mid) {
                                                                            fetch(`/api/program/session?memberId=${encodeURIComponent(mid)}&tz=${encodeURIComponent(Intl.DateTimeFormat().resolvedOptions().timeZone)}`).then(r => r.json()).then(vd2 => {
                                                                                if (vd2.active) {
                                                                                    console.log('[program] refreshed data, programTasks submitted states:', vd2.programTasks?.map((t: any) => `${t.type}:${t.submitted || 'none'}`));
                                                                                    setProgramData(vd2);
                                                                                }
                                                                            }).catch(() => {});
                                                                        }
                                                                    } catch (e: any) { console.error('[program] submit_task fetch error:', e); alert('Submit failed: ' + e?.message); }
                                                                };
                                                                ...
                                                                return (
                                                                    <div>
                                                                            ...
                                                                            if (simonPhase === 'waiting') return (
                                                                                <div style={{ textAlign: 'center', padding: '36px 10px 24px', animation: 'vFadeIn 0.8s ease' }}>
                                                                                    ...
                                                                                    <div style={{
                                                                                        ...
                                                                                        background: `${R}0.75)`, margin: '0 auto 32px',
                                                                                        animation: 'vPulse 1.5s ease infinite',
                                                                                        boxShadow: `0 0 16px ${R}0.3)`,
                                                                                    }} />
                                                                                    ...
                                                                                    <div style={{ marginTop: 36, fontFamily: 'Orbitron, sans-serif', fontSize: '0.48rem', color: 'rgba(255,255,255,0.1)', letterSpacing: '4px' }}>
                                                                                        TASK {simonStep + 1} / {chain.length}
                                                                                    </div>
                                                                                </div>
                                                                            );
                                                                            ...
                                                                            if (simonPhase === 'task' && simonCurrentTask) {
                                                                                ...
                                                                                return (
                                                                                    <div style={{ animation: 'vFadeIn 0.4s ease' }}>
                                                                                        ...
                                                                                        <div style={{
                                                                                            fontFamily: 'Cinzel, serif', fontSize: '1rem', color: 'rgba(255,255,255,0.88)', lineHeight: 1.75,
                                                                                            marginBottom: 18, padding: '18px 16px',
                                                                                            background: `${R}0.06)`, border: `1px solid ${R}0.2)`, borderRadius: 10, textAlign: 'center',
                                                                                        }}>{simonCurrentTask.text}</div>
                                                                                        <div style={{ marginBottom: 20 }}>
                                                                                            <div style={{ height: 3, background: 'rgba(255,255,255,0.04)', borderRadius: 2, overflow: 'hidden' }}>
                                                                                                <div style={{
                                                                                                    height: '100%',
                                                                                                    width: `${simonTaskLimit > 0 ? (simonTaskSecs / simonTaskLimit) * 100 : 100}%`,
                                                                                                    background: simonTaskSecs <= Math.ceil(simonTaskLimit * 0.2) ? 'rgba(255,50,50,0.7)' : `${R}0.55)`,
                                                                                                    borderRadius: 2, transition: 'width 0.9s linear, background 0.4s',
                                                                                                }} />
                                                                                            </div>
                                                                                        </div>
                                                                                        <label style={{ cursor: simonUploading ? 'default' : 'pointer', display: 'block' }}>
                                                                                            ...
                                                                                            <input type="file" accept={isVideo ? 'video/*' : 'image/*'} capture="environment" style={{ display: 'none' }} onChange={async (e) => {
                                                                                                ...
                                                                                                try {
                                                                                                    ...
                                                                                                    fd.append('file', file);
                                                                                                    fd.append('folder', `program/simon/${mid}`);
                                                                                                    ...
                                                                                                    if (uploadData.url) {
                                                                                                        ...
                                                                                                        if (nextStep >= chain.length) {
                                                                                                            ...
                                                                                                            setSimonStep(0); setSimonProofs([]); setSimonCurrentTask(null); setSimonLastTask(null);
                                                                                                            setSimonPhase('complete');
                                                                                        ...
                                                                                        <div style={{ marginTop: 10, fontFamily: 'Orbitron, sans-serif', fontSize: '0.45rem', color: 'rgba(255,255,255,0.1)', letterSpacing: '4px', textAlign: 'center' }}>
                                                                                            TASK {simonStep + 1} / {chain.length}
                                                                                        </div>
                                                                                    </div>
                                                                                );
                                                                                ...
                                                                                <label style={{ cursor: 'pointer' }}>
                                                                                    ...
                                                                                    <input type="file" accept="image/*,video/*" capture="environment" style={{ display: 'none' }} onChange={async (e) => {
                                                                                        ...
                                                                                        try {
                                                                                            ...
                                                                                            const fd = new FormData(); fd.append('file', file); fd.append('folder', `program/tasks/${mid}`); fd.append('ext', ext === 'heic' ? 'jpg' : ext);
                                                                                            const res = await fetch('/api/upload', { method: 'POST', body: fd });
                                                                                            const data = await res.json();
                                                                        ...
                                                                        <div style={{ marginTop: 20, paddingTop: 14, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                                                                            <button onClick={() => setProgramSkipOpen(true)} style={{
            ...
            {followUp && (() => {
                ...
                return (
                    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'linear-gradient(rgba(4,3,10,0.78) 0%, rgba(4,3,10,0.88) 100%), url(/work-bg.jpg) center top / cover no-repeat', display: 'flex', flexDirection: 'column', overflow: 'auto' } as React.CSSProperties}>
                        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0, background: 'radial-gradient(ellipse at 50% 20%, rgba(197,160,89,0.08) 0%, transparent 60%)' }} />
                            ...
                            {followUpSkipping && (
                                <div style={{ position: 'absolute', inset: 0, zIndex: 20, background: 'rgba(5,5,8,0.95)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 30, gap: 24, animation: 'vFadeIn 0.3s ease' }}>
                                    ...
                                    <button onClick={async () => {
                                        ...
                                        try {
                                            const res = await fetch('/api/program/session', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'skip_order', memberId: mid, orderType: followUp.orderType, cost: 300 }) });
                                            const data = await res.json();
                                            if (data.success) {
                                                setProfile((p: any) => ({ ...p, wallet: (p?.wallet || 0) - 300 }));
                                                setFollowUp(null); setFollowUpText(''); setFollowUpSkipping(false);
                                                if (mid) { fetch(`/api/program/session?memberId=${encodeURIComponent(mid)}`).then(r => r.json()).then(vd2 => { if (vd2.active) setProgramData(vd2); }); }
                                            }
                                        } catch {}
                                    }} style={{
                                    ...
                                    <button disabled={!(profile?.skippass > 0)} onClick={async () => {
                                        try {
                                            const res = await fetch('/api/program/session', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'skip_order', memberId: mid, orderType: followUp.orderType, useSkipPass: true }) });
                                            const data = await res.json();
                                            if (data.success) {
                ...
                return (
                    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'linear-gradient(rgba(4,3,10,0.88) 0%, rgba(4,3,10,0.94) 100%), url(/work-bg.jpg) center top / cover no-repeat', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 30, gap: 24, animation: 'vFadeIn 0.25s ease' }}>
                        ...
                        <button onClick={async () => {
                            const coins = profile?.wallet ?? 0;
                            if (coins < 300) { alert('Not enough coins. 300 required.'); return; }
                            if (!currentSkipTask) { setProgramSkipOpen(false); return; }
                            try {
                                const res = await fetch('/api/program/session', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'skip_order', memberId: skipMid, orderType: currentSkipTask.type, cost: 300 }) });
                                const data = await res.json();
                                if (data.success) {
                                    setProfile((p: any) => ({ ...p, wallet: (p?.wallet || 0) - 300 }));
                                    setTaskSubmitted(prev => ({ ...prev, [currentSkipTask.type]: true }));
                                    setProgramSkipOpen(false);
                                    if (skipMid) { fetch(`/api/program/session?memberId=${encodeURIComponent(skipMid)}&tz=${encodeURIComponent(Intl.DateTimeFormat().resolvedOptions().timeZone)}`).then(r => r.json()).then(vd2 => { if (vd2.active) setProgramData(vd2); }).catch(() => {}); }
                        ...
                        <button disabled={!(profile?.skippass > 0)} onClick={async () => {
                            if (!currentSkipTask) { setProgramSkipOpen(false); return; }
                            try {
                                const res = await fetch('/api/program/session', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'skip_order', memberId: skipMid, orderType: currentSkipTask.type, useSkipPass: true }) });
                                const data = await res.json();
                                if (data.success) {
                                    setProfile((p: any) => ({ ...p, skippass: Math.max(0, (p?.skippass || 0) - 1) }));
                                    setTaskSubmitted(prev => ({ ...prev, [currentSkipTask.type]: true }));
                        ...
                        <button onClick={() => setProgramSkipOpen(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'Rajdhani, sans-serif', fontSize: '0.75rem', color: 'rgba(255,255,255,0.18)', letterSpacing: 2, padding: '12px', marginTop: 8 }}>CANCEL</button>
                    </div>
                );
            })()}
                        ...
                        {!begSent ? (
                            <>
                                ...
                                <button onClick={() => {
                                    ...
                                    // Record beg in DB
                                    if (programData?.session?.id) {
                                        fetch('/api/program/session', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'beg', memberId: profile?.member_id || profile?.memberId || '', message: begText }) }).catch(() => {});
                                    }
                    ...
                    <div style={{ width: '100%', maxWidth: 400, maxHeight: '85vh', overflowY: 'auto', background: '#0a0a0e', border: `1px solid ${R}0.15)`, borderRadius: 16, padding: '24px 20px' }} onClick={e => e.stopPropagation()}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                ...
                                <div style={{
                                    ...
                                    {selectedDay.perfect
                                        ? <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="#c5a059" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>
                        ...
                        <div style={{ marginBottom: 20 }}>
                            ...
                            {selectedDay.orders.map((o, i) => {
                                const completed = o.done >= o.target;
                                const label = o.type === 'kneel' ? `Kneel ${o.target} times` : o.type === 'daily_check' ? 'Chastity check photo' : o.type === 'spin' ? 'Spin the wheel' : o.type === 'trial' ? 'Complete daily trial' : o.type === 'tribute' ? `Tribute ${o.target} coins` : o.type === 'silence' ? 'No messages today' : o.type;
                                return (
                                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                                        <div style={{
                                            ...
                                            {completed && <svg viewBox="0 0 24 24" width="10" height="10" fill="none" stroke="#c5a059" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>}
                                        </div>
            ...
            <nav id="mobBottomNav" className="mob-bottom-nav" onClick={() => { const _ov = document.getElementById('mobChatOverlay'); if (_ov?.classList.contains('mob-overlay-open')) (window as any).closeMobChatOverlay?.(); }} style={{
                ...
                <NavBtn active={tab === 'program'} icon="&#9670;" label="PROGRAM" onClick={() => { setVladOpen(false); setTab('program'); }} />
                ...
                <div style={{ position: 'fixed', bottom: 90, right: 12, zIndex: 150, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 10 }}>
                    ...
                    {vladBubble && (
                        <div onClick={() => { setVladOpen(true); setVladPulse(false); setVladBubble(''); }} style={{
                            ...
                            boxShadow: '0 10px 40px rgba(0,0,0,0.8), 0 0 20px rgba(197,160,89,0.06), inset 0 1px 0 rgba(197,160,89,0.05)',
                    ...
                    <button onClick={() => { setVladOpen(true); setVladPulse(false); setVladBubble(''); if (vladMsgs.length === 0) vladReact('Member just opened the chat for the first time today. Greet them — you can see they\'re locked up. Be sarcastic but welcoming.'); }} style={{
                        ...
                        border: vladPulse ? '2.5px solid rgba(197,160,89,0.7)' : '2px solid rgba(197,160,89,0.45)',
                        cursor: 'pointer', overflow: 'hidden', flexShrink: 0,
                        boxShadow: vladPulse
                            ? '0 0 24px rgba(197,160,89,0.35), 0 0 8px rgba(197,160,89,0.4), 0 4px 20px rgba(0,0,0,0.6)'
            ...
            {vladOpen && (
                <div onClick={e => e.stopPropagation()} onPointerDown={e => e.stopPropagation()} onTouchStart={e => e.stopPropagation()} style={{
                    ...
                    {/* BG glow */}
                    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'radial-gradient(ellipse at 50% 15%, rgba(197,160,89,0.04) 0%, transparent 55%), radial-gradient(ellipse at 80% 80%, rgba(197,160,89,0.02) 0%, transparent 50%)' }} />
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                            <div style={{ width: 48, height: 48, borderRadius: '50%', overflow: 'hidden', border: '2px solid rgba(197,160,89,0.5)', flexShrink: 0, boxShadow: '0 0 18px rgba(197,160,89,0.15), 0 0 6px rgba(197,160,89,0.2)' }}>
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                    ...
                    <div ref={vladScrollRef} style={{ flex: 1, overflowY: 'auto', padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 14, position: 'relative', zIndex: 1 }}>
                        {vladMsgs.map((m, i) => (
                            <div key={i} style={{
                                ...
                                {m.role === 'vlad' && (
                                    <div style={{ width: 32, height: 32, borderRadius: '50%', overflow: 'hidden', border: '1.5px solid rgba(197,160,89,0.35)', flexShrink: 0, boxShadow: '0 0 8px rgba(197,160,89,0.08)' }}>
                                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        ...
                        {vladSending && (
                            <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end' }}>
                                ...
                                <div style={{ padding: '12px 16px', borderRadius: '16px 16px 16px 4px', background: 'rgba(197,160,89,0.04)', border: '1px solid rgba(197,160,89,0.08)' }}>
                    ...
                    <div style={{
                        ...
                        <button
                            ...
                            style={{
                                padding: '10px 18px', background: 'rgba(197,160,89,0.08)',
            ...
            <style>{`
                ...
                @keyframes vladPulse { 0%, 100% { box-shadow: 0 0 12px rgba(197,160,89,0.15), 0 0 4px rgba(197,160,89,0.3); } 50% { box-shadow: 0 0 28px rgba(197,160,89,0.35), 0 0 10px rgba(197,160,89,0.45); } }
                * { box-sizing: border-box; }
                ::-webkit-scrollbar { display: none; }
                * { -ms-overflow-style: none; scrollbar-width: none; }
                textarea:focus, input:focus { border-color: rgba(197,160,89,0.25) !important; }
                body { background-image: none !important; background-color: #080810 !important; }

                /* Program timer boxes: red override */
                .card-t-box {
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
                                ...
                                <div style={{ width: '100%', maxWidth: 340, display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 36 }}>
                                    <div style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '0.6rem', fontWeight: 700, color: 'rgba(255,255,255,0.55)', letterSpacing: 3 }}>
                                        DAY&nbsp;
                                        <span style={{ color: 'rgba(255,255,255,0.2)', fontWeight: 400, fontSize: '0.5rem' }}>{programData?.session?.day_number ?? 1}</span>
                                        &nbsp;<span style={{ color: 'rgba(255,255,255,0.2)', fontWeight: 400, fontSize: '0.5rem' }}>OF {programData?.session?.total_days ?? 30}</span>
                                    </div>
                                ...
                                {/* Submit */}
                                <button
                                    disabled={!gatePreviewUrl || dailyUploading}
                                    onClick={async () => {
                                        if (!gateFile || dailyUploading) return;
                                        ...
                                        try {
                                            ...
                                            fd.append('folder', `program/daily/${mid}`);
                                            fd.append('ext', ext === 'heic' ? 'jpg' : ext);
                                            const res = await fetch('/api/upload', { method: 'POST', body: fd });
                                            const uploadData = await res.json();
                                            if (uploadData.url && programData?.session?.id) {
                                                const tz = Intl.DateTimeFormat().resolvedOptions().timeZone;
                                                const submitRes = await fetch('/api/program/session', {
                                                    method: 'POST',
                                                    headers: { 'Content-Type': 'application/json' },
                                                    body: JSON.stringify({ action: 'complete_order', memberId: mid, orderType: 'daily_check', photoUrl: uploadData.url, tz }),
                                                });
                                                if (submitRes.ok) {
                                                    ...
                                                    vladReact('Member just submitted their daily daily check photo. Good boy — or is he hiding something?');
                                                    setTimeout(() => {
                                                        setShowChastityGate(false);
                                    ...
                                    style={{
                                        ...
                                        border: (gatePreviewUrl && !dailyUploading) ? '1px solid rgba(160,0,0,0.3)' : '1px solid rgba(255,255,255,0.06)',
                                        borderRadius: 12, background: (gatePreviewUrl && !dailyUploading) ? 'rgba(160,0,0,0.06)' : 'transparent',
                                        fontFamily: 'Orbitron, sans-serif', fontSize: '0.62rem', letterSpacing: 5,
                                        color: dailyUploading ? 'rgba(255,255,255,0.2)' : gatePreviewUrl ? 'rgba(255,255,255,0.7)' : 'rgba(255,255,255,0.12)',
                                        cursor: (gatePreviewUrl && !dailyUploading) ? 'pointer' : 'not-allowed',
                                        boxShadow: (gatePreviewUrl && !dailyUploading) ? '0 0 30px rgba(120,0,0,0.12), inset 0 0 20px rgba(120,0,0,0.04)' : 'none',
                                    }}
                                >{dailyUploading ? 'UPLOADING...' : 'SUBMIT PROOF'}</button>
...
function NavBtn({ active, icon, label, onClick, locked }: { active: boolean; icon: React.ReactNode; label: string; onClick: () => void; locked?: boolean }) {
    const color = active ? `rgba(197,160,89,0.7)` : 'rgba(255,255,255,0.2)';
    return (
        <button onClick={onClick} style={{ background: 'none', border: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, cursor: 'pointer', padding: '8px 0', position: 'relative', flex: 1 }}>
            ...
            {locked && (
                <div style={{ position: 'absolute', top: -2, right: 2, width: 12, height: 12, borderRadius: '50%', background: '#050508', border: '1px solid rgba(197,160,89,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <svg viewBox="0 0 24 24" width="7" height="7" fill="rgba(197,160,89,0.4)"><rect x="3" y="11" width="18" height="11" rx="2" /><path d="M7 11V7a5 5 0 0110 0v4" /></svg>
                </div>
            )}
        </button>
    );
