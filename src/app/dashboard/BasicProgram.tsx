...
import { kneelTarget, defaultDayTasks, generateDefaultProgram } from '@/lib/program-defaults';
...
const RED = 'rgba(197,160,89,0.9)';
const RED_DIM = 'rgba(197,160,89,0.25)';
const BG = '#0a0a10';
...
/* RED #c5a059   = punishment / intensity / sacrifice */
...
const TASK_META: Record<string, { label: string; icon: string; color: string; configKey?: string }> = {
    ...
    daily_check: { label: 'CHASTITY',     icon: '\u25C8', color: C_RED },
...
const PHASES = [
    ...
    { name: 'DISCIPLINE', sub: 'Building', days: [8,9,10,11,12,13,14], color: '#c5a059' },
...
// _kt and _ddt now imported from @/lib/program-defaults
...
/* ═══════════════ MAIN ═══════════════ */
export function BasicProgramProgramContent({ onClose, initialMember }: { onClose: () => void; initialMember?: string }) {
    ...
    const loadLockedMembers = async () => { try { const r = await fetch('/api/program/program?listLocked=true'); const j = await r.json(); if(j.locked) setLockedMembers(j.locked); } catch{} };
    const loadTemplate = async () => { try { const r = await fetch('/api/program/program?template=true'); const j = await r.json(); if(j.template?.length>0){ const d: Record<string,Task[]>={}; for(const row of j.template){ const tasks: Task[] = typeof row.tasks==='string'?JSON.parse(row.tasks):row.tasks; const dn = row.day_number; if(!tasks.some((t:Task)=>t.type==='kneel')){const kt=_kt(dn);tasks.unshift({type:'kneel',target:kt,label:`Kneel ${kt} times`} as Task);} if(!tasks.some((t:Task)=>t.type==='daily_check')){const ki=tasks.findIndex((t:Task)=>t.type==='kneel');tasks.splice(ki+1,0,{type:'daily_check',target:1,label:'Chastity check-in'} as Task);} d[String(dn)]=tasks; } setTemplateDays(d); } else { /* DB has no template — auto-save the defaults so member program generation uses the same data */ const defaults = generateDefaultProgram() as Record<string,Task[]>; try { await fetch('/api/program/program',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'save_template',days:defaults})}); console.log('[BasicProgramProgram] Auto-saved default template to DB'); } catch{} } } catch{} };
    const loadConfig = async () => { try { const r = await fetch('/api/program/program?config=true'); const j = await r.json(); if(j.config){ const m: Record<string,any>={}; for(const row of j.config){ m[row.key]=typeof row.value==='string'?JSON.parse(row.value):row.value; } setConfigData(m); } } catch{} };
    const loadMemberProgram = async (emailOverride?: string) => { const email = emailOverride || memberEmail; if(!email) return; setLoading(true); setMemberAbout(''); try { const r = await fetch(`/api/program/program?memberId=${encodeURIComponent(email)}`); const j = await r.json(); if(j.program?.program){const mp=typeof j.program.program==='string'?JSON.parse(j.program.program):j.program.program;for(const[ds,tasks]of Object.entries(mp)){if(!Array.isArray(tasks))continue;const dn=parseInt(ds,10);if(!(tasks as Task[]).some((t:Task)=>t.type==='kneel')){const kt=_kt(dn);(tasks as Task[]).unshift({type:'kneel',target:kt,label:`Kneel ${kt} times`} as Task);}if(!(tasks as Task[]).some((t:Task)=>t.type==='daily_check')){const ki=(tasks as Task[]).findIndex((t:Task)=>t.type==='kneel');(tasks as Task[]).splice(ki+1,0,{type:'daily_check',target:1,label:'Chastity check-in'} as Task);}}setMemberProgram(mp);}else{setMemberProgram(null);} const base = lockedMembers.find((m:any)=>m.memberId.toLowerCase()===email.toLowerCase())||{}; if(j.profile) { base.kinks = j.profile.kinks || ''; base.limits = j.profile.limits || ''; } setMemberInfo(base); } catch{} try { const cr = await fetch(`/api/chat/history`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ memberId: email }) }); const cj = await cr.json(); if(cj.messages) { const firstUserMsg = cj.messages.find((m: any) => m.sender_email === email && m.content && !m.content.startsWith('http') && !m.content.startsWith('[SYSTEM') && !m.content.startsWith('[system') && m.type !== 'system' && m.type !== 'wishlist' && m.content.length > 20); if(firstUserMsg) setMemberAbout(firstUserMsg.content); } } catch{} setLoading(false); };
    const generateMemberProgram = async () => { if(!memberEmail) return; setSaving(true); try { await fetch('/api/program/program',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'generate_program',memberId:memberEmail})}); await loadMemberProgram(); } catch{} setSaving(false); };
    const saveTemplate = async () => { setSaving(true); try { await fetch('/api/program/program',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'save_template',days:templateDays})}); } catch{} setSaving(false); };
    const saveConfig = async (key: string, value: any) => { setSaving(true); try { await fetch('/api/program/program',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'save_config',key,value})}); } catch{} setSaving(false); };
    const saveMemberDay = async (dayNum: number, tasks: Task[]) => { if(!memberEmail) return; setSaving(true); try { await fetch('/api/program/program',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'update_day',memberId:memberEmail,dayNumber:dayNum,tasks})}); } catch{} setSaving(false); };
    ...
    const autoSaveMemberDay = (dn: number, tasks: Task[]) => {
        ...
        saveTimerRef.current = setTimeout(async () => {
            try { await fetch('/api/program/program',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'update_day',memberId:memberEmailRef.current,dayNumber:dn,tasks})}); } catch{}
        }, 600);
    };
    ...
    return (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: BG, overflow: 'hidden', fontFamily: F, position: 'relative' }}>
            {/* Full background image — same as /basic_program page */}
            ...
            <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 0%, rgba(197,160,89,0.1) 0%, transparent 60%)', pointerEvents: 'none', zIndex: 0 }} />
            <style>{CSS}</style>
            ...
            <div style={{ display: 'flex', alignItems: 'center', padding: '14px 28px', borderBottom: `1px solid rgba(197,160,89,.18)`, gap: 16, flexShrink: 0, position: 'relative', zIndex: 1, background: 'rgba(10,10,16,0.85)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)' }}>
                ...
                <h1 style={{ fontFamily: FC, fontSize: '.8rem', color: GOLD, letterSpacing: 8, margin: 0, flex: 1, textTransform: 'uppercase' }}>BASIC PROGRAM</h1>
                    ...
                    {lockedMembers.map((m: any) => (
                        <div key={m.memberId} className="kmc"
                            ...
                            {m.avatar ? (
                                ...
                                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, rgba(197,160,89,.2), rgba(20,16,28,.9))' }} />
                            )}
                            {/* Content overlay */}
                                ...
                                <div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                        <div style={{ flex: 1, height: 3, background: 'rgba(197,160,89,.15)', borderRadius: 2, overflow: 'hidden' }}>
                                            <div style={{ width: `${m.lockDays?(m.daysIn/m.lockDays)*100:0}%`, height: '100%', background: `linear-gradient(90deg, ${GOLD}, rgba(197,160,89,.7))`, borderRadius: 2 }} />
                                        </div>
                                    ...
                                    <div key={d} className="kdc" onClick={() => setSel(isA?null:d)} style={{
                                        ...
                                        {!sel && (
                                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 4 }}>
                                                <span style={{ fontSize: '.6rem', color: 'rgba(197,160,89,.7)' }}>{'\u25C8'}</span>
        ...
        <div className="kslide" style={{ width: '68%', borderLeft: `1px solid rgba(197,160,89,.18)`, display: 'flex', flexDirection: 'column', background: 'rgba(10,10,16,0.92)', overflow: 'hidden', position: 'relative' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '24px 36px', position: 'relative', zIndex: 1 }} className="kscr">
                ...
                <div style={{
                    ...
                    background: 'rgba(197,160,89,.08)', border: '1px solid rgba(197,160,89,.25)',
                    boxShadow: '0 4px 20px rgba(0,0,0,.3)',
                }}>
                    <div style={{ width: 40, height: 40, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(197,160,89,.15)', border: '1px solid rgba(197,160,89,.3)' }}>
...
function MemberView({ email, setEmail, program, sel, setSel, info, locked, onLoad, onGenerate, updateTask, addTask, removeTask, moveTask, saveMemberDay, saving, loading, dragIdx, setDragIdx, configData, setView, setConfigSection, memberAbout, showMemberProfile, setShowMemberProfile }: any) {
    ...
    const addLockDays = async () => {
        ...
        try {
            await fetch('/api/program/session', {
                method: 'POST',
                            ...
                            <div key={m.memberId} className="kmc kfade" onClick={() => { setEmail(m.memberId); setTimeout(()=>onLoad(m.memberId),80); }} style={{
                                ...
                                <div style={{ position: 'relative', zIndex: 1, padding: '16px 20px', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                        <div style={{ flex: 1, height: 3, background: 'rgba(197,160,89,.15)', borderRadius: 2, overflow: 'hidden' }}>
                                            <div style={{ width: `${m.lockDays?(m.daysIn/m.lockDays)*100:0}%`, height: '100%', background: `linear-gradient(90deg, ${GOLD}, rgba(197,160,89,.6))` }} />
                                        </div>
                    ...
                    <div style={{ width: sel?'32%':'100%', transition: 'width .4s ease', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'rgba(10,10,16,0.5)' }}>
                        <div style={{ padding: '14px 24px', borderBottom: `1px solid rgba(197,160,89,.18)`, display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0, background: 'rgba(10,10,16,0.6)' }}>
                            ...
                            {loading ? (
                                ...
                                <button onClick={onGenerate} className="kbtn" style={{ padding: '9px 22px', borderRadius: 6, border: `1px solid rgba(197,160,89,.3)`, background: RED_DIM, color: RED, fontFamily: FC, fontSize: '.4rem', letterSpacing: 3 }}>{saving?'REGENERATING...':'REGENERATE'}</button>
                            ) : (
                                            ...
                                            <div style={{ marginBottom: 12 }}>
                                                ...
                                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                                                    {info.limits.split(',').map((k: string) => k.trim()).filter(Boolean).map((k: string) => (
                                                        <span key={k} style={{ padding: '3px 10px', borderRadius: 12, border: '1px solid rgba(197,160,89,.3)', background: 'rgba(197,160,89,.06)', fontFamily: F, fontSize: '.4rem', color: 'rgba(200,60,60,.8)', letterSpacing: 1 }}>{k}</span>
                                                    ))}
                                                </div>
                                            </div>
                                        )}
                                            ...
                                            <div style={{ display: 'grid', gridTemplateColumns: sel?'1fr 1fr':'repeat(auto-fill,minmax(120px,1fr))', gap: 8 }}>
                                                {phase.days.map(d => {
                                                    ...
                                                    return (<div key={d} className="kdc" onClick={() => setSel(isA?null:d)} style={{
                                                        ...
                                                        border: `1px solid ${isA?'rgba(197,160,89,.35)':isC?'rgba(197,160,89,.3)':isEmpty?'rgba(255,255,255,.06)':'rgba(197,160,89,.12)'}`,
                                                        background: isA?'rgba(197,160,89,.1)':isC?'rgba(197,160,89,.08)':'rgba(22,20,30,.95)',
