                    ...
                    {chatMsgs.map((msg, i) => {
                        ...
                        if (text.startsWith('WELCOME_CARD::')) {
                            try {
                                ...
                                return (
                                    <div key={msg.id || i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '4px 0' }}>
                                        <div style={{ width: 260, maxWidth: '72vw', borderRadius: 16, overflow: 'hidden', background: 'linear-gradient(170deg,#0c0a04 0%,#13100a 50%,#0c0a04 100%)', border: '1px solid rgba(197,160,89,0.6)', boxShadow: '0 12px 40px rgba(0,0,0,0.8),0 0 30px rgba(197,160,89,0.08)' }}>
                                            <div style={{ width: '100%', padding: '18px 0 12px', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'radial-gradient(ellipse at center top,rgba(197,160,89,0.1) 0%,transparent 70%)' }}>
                                                <div style={{ width: 56, height: 56, borderRadius: '50%', border: '2px solid rgba(197,160,89,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Cinzel,serif', fontSize: '1.4rem', color: '#c5a059', background: 'radial-gradient(circle,rgba(197,160,89,0.12) 0%,rgba(197,160,89,0.03) 100%)', boxShadow: '0 0 20px rgba(197,160,89,0.15),0 0 40px rgba(197,160,89,0.05)' }}>{wIni}</div>
                                            </div>
                                            <div style={{ padding: '4px 16px 18px', textAlign: 'center' }}>
                                                <div style={{ fontFamily: 'Cinzel,serif', fontSize: '0.95rem', color: '#fff', fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase' as const, marginBottom: 6 }}>{d.name}</div>
                                                <div style={{ width: '40%', height: 1, background: 'linear-gradient(to right,transparent,rgba(197,160,89,0.5),transparent)', margin: '0 auto 8px' }} />
                                                <div style={{ fontFamily: 'Orbitron,sans-serif', fontSize: '0.38rem', color: 'rgba(197,160,89,0.65)', letterSpacing: 3, marginBottom: 10 }}>HAS ENTERED THE COURT</div>
                                                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'rgba(197,160,89,0.06)', border: '1px solid rgba(197,160,89,0.25)', borderRadius: 20, padding: '3px 12px' }}>
                                                    <svg width="13" height="10" viewBox="0 0 26 20" fill="#c5a059"><path d="M2 18 L5 8 L10 13 L13 3 L16 13 L21 8 L24 18 Z"/><rect x="2" y="17" width="22" height="2" rx="1"/></svg>
                                                    <span style={{ fontFamily: 'Orbitron,sans-serif', fontSize: '0.42rem', color: '#c5a059', letterSpacing: 2 }}>{(d.rank || 'HALL BOY').toUpperCase()}</span>
                                                </div>
                                            </div>
                                        </div>
                                        <span style={{ fontFamily: 'Orbitron,monospace', fontSize: '0.62rem', color: 'rgba(197,160,89,0.35)', marginTop: 4 }}>{timeStr}</span>
                        ...
                        if (text.startsWith('TASK_REVIEW_CARD::')) {
                            try {
                                const d = JSON.parse(text.replace('TASK_REVIEW_CARD::', ''));
                                ...
                                return (
                                    <div key={msg.id || i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '4px 0' }}>
                                        <div style={{ width: '82%', maxWidth: 260, borderRadius: 14, overflow: 'hidden', background: 'linear-gradient(170deg,#0e0b06,#110d04,#0a0703)', border: `1px solid ${borderColor}`, boxShadow: '0 12px 40px rgba(0,0,0,0.8)' }}>
                                            <div style={{ padding: '18px 20px', textAlign: 'center' }}>
                                                <div style={{ fontFamily: "'Cinzel',serif", fontSize: '0.42rem', color: accentColor, letterSpacing: '3px', marginBottom: 10 }}>{label} {statusText}</div>
                                                <div style={{ width: '40%', height: 1, background: `linear-gradient(to right,transparent,${borderColor},transparent)`, margin: '0 auto 12px' }} />
                                                {d.thumbnail ? <img src={d.thumbnail} style={{ width: 52, height: 52, borderRadius: 6, objectFit: 'cover', border: '1px solid rgba(255,255,255,0.08)', marginBottom: 10 }} onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} alt="" /> : null}
                                                {d.taskText ? <div style={{ fontFamily: 'Rajdhani,sans-serif', fontSize: '0.48rem', color: 'rgba(255,255,255,0.35)', letterSpacing: '1px', marginBottom: 8, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' } as any}>{d.taskText}</div> : null}
                                                <div style={{ fontSize: '1.6rem', marginBottom: 8 }}>{icon}</div>
                                                {approved && d.points ? <div style={{ fontFamily: "'Cinzel',serif", fontSize: '0.75rem', color: accentColor, fontWeight: 700, letterSpacing: '1px' }}>+{(d.points || 0).toLocaleString()} MERIT</div> : null}
                                                {d.penalty ? <div style={{ fontFamily: "'Cinzel',serif", fontSize: '0.75rem', color: accentColor, fontWeight: 700, letterSpacing: '1px' }}>-{d.penalty} COINS</div> : null}
                                                {d.comment ? <div style={{ fontFamily: 'Rajdhani,sans-serif', fontSize: '0.45rem', color: 'rgba(255,255,255,0.4)', marginTop: 6, fontStyle: 'italic' }}>&quot;{d.comment}&quot;</div> : null}
                                            </div>
                                        </div>
                                        <span style={{ fontFamily: 'Orbitron,monospace', fontSize: '0.62rem', color: 'rgba(197,160,89,0.35)', marginTop: 4 }}>{timeStr}</span>
                                    </div>
                        ...
                        if (text.startsWith('VAULT_LOCK_CARD::')) {
                            try {
                                const d = JSON.parse(text.replace('VAULT_LOCK_CARD::', ''));
                                const typeLabel = d.type === 'instant' ? 'Self-locked' : 'Awaiting approval';
                                return (
                                    <div key={msg.id || i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '4px 0' }}>
                                        <div style={{ width: '75%', maxWidth: 240, borderRadius: 14, overflow: 'hidden', background: 'linear-gradient(170deg,#0e0406,#0d0404,#0a0303)', border: '1px solid rgba(139,0,0,0.4)', boxShadow: '0 12px 40px rgba(0,0,0,0.8)' }}>
                                            <div style={{ padding: '16px 20px', textAlign: 'center' }}>
                                                <div style={{ fontFamily: "'Cinzel',serif", fontSize: '0.75rem', color: 'rgba(139,0,0,0.65)', letterSpacing: '3px', marginBottom: 8 }}>KEYHOLDER LOCK</div>
                                                <div style={{ width: '40%', height: 1, background: 'linear-gradient(to right,transparent,rgba(139,0,0,0.35),transparent)', margin: '0 auto 10px' }} />
                                                <div style={{ fontSize: '1.2rem', marginBottom: 8 }}>🔏</div>
                                                <div style={{ fontFamily: "'Cinzel',serif", fontSize: '0.85rem', color: 'rgba(255,255,255,0.6)', letterSpacing: '1px', marginBottom: 4 }}>{d.name || ''} — {d.days || 0} Days</div>
                                                <div style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: '0.7rem', color: 'rgba(139,0,0,0.45)' }}>{typeLabel}</div>
                                            </div>
                                        </div>
