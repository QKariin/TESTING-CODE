...
export function KeyholderProgramContent({ onClose, initialMember }: { onClose: () => void; initialMember?: string }) {
    ...
    const loadMemberProgram = async () => { if(!memberEmail) return; setLoading(true); try { const r = await fetch(`/api/vault/program?memberId=${encodeURIComponent(memberEmail)}`); const j = await r.json(); if(j.program?.program){setMemberProgram(typeof j.program.program==='string'?JSON.parse(j.program.program):j.program.program);}else{ /* No program found — auto-generate from template */ try { await fetch('/api/vault/program',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'generate_program',memberId:memberEmail})}); const r2 = await fetch(`/api/vault/program?memberId=${encodeURIComponent(memberEmail)}`); const j2 = await r2.json(); if(j2.program?.program){setMemberProgram(typeof j2.program.program==='string'?JSON.parse(j2.program.program):j2.program.program);}else{setMemberProgram(null);} } catch{ setMemberProgram(null); } } setMemberInfo(lockedMembers.find((m:any)=>m.memberId.toLowerCase()===memberEmail.toLowerCase())||null); } catch{} setLoading(false); };
