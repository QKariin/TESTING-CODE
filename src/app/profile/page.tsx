...
export default function ProfilePage() {
        ...
        async function loadProfile() {
            ...
            try {
                ...
                if (unifiedData) {
                    ...
                    // ── PAYWALL: kill everything immediately, render only the wall ──
                    if (unifiedData?.parameters?.paywall?.active) {
                        setPaywallData({
                            paywall: unifiedData.parameters.paywall,
                            memberId: unifiedData.member_id || unifiedData.memberId || '',
                        });
                        setLoading(false); // drop splash immediately
                        return; // stop — nothing else loads, no subscriptions, no intervals, nothing
                    }
    ...
    // ── PAYWALL GATE: render absolutely nothing except the wall ──
    if (paywallData) {
        const pw = paywallData.paywall;
        const mid = paywallData.memberId;
        return (
            <div style={{ position: 'fixed', inset: 0, background: 'rgba(2,5,18,1)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px', fontFamily: 'sans-serif' }}>
                <link href="https://fonts.googleapis.com/css2?family=Cinzel:wght@400;700&family=Orbitron:wght@400;700&family=Rajdhani:wght@400;500;600&display=swap" rel="stylesheet" />
                <div style={{ maxWidth: 420, width: '100%', textAlign: 'center' }}>
                    ...
                    <div style={{ background: 'rgba(197,160,89,0.05)', border: '1px solid rgba(197,160,89,0.25)', borderRadius: 14, padding: '28px 24px', marginBottom: 20 }}>
                        ...
                        <div style={{ fontFamily: 'Dancing Script,cursive', fontSize: '1.15rem', color: '#fff', lineHeight: 1.6 }}>{pw.reason || ''}</div>
                    ...
                    <div style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', background: 'rgba(139,0,0,0.06)', border: '1px solid rgba(139,0,0,0.18)', borderRadius: 10, marginBottom: 12, textAlign: 'left' }}>
                        <div style={{ flexShrink: 0, width: 32, height: 32, borderRadius: '50%', background: 'rgba(139,0,0,0.12)', border: '1px solid rgba(139,0,0,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="rgba(180,40,40,0.7)" strokeWidth="1.5"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>
                        </div>
                        <div>
                            <div style={{ fontFamily: 'Cinzel,serif', fontSize: '0.5rem', color: 'rgba(180,40,40,0.7)', letterSpacing: '3px', marginBottom: 3 }}>CARD PAYMENT UNAVAILABLE</div>
                            <div style={{ fontFamily: 'Rajdhani,sans-serif', fontSize: '0.68rem', color: 'rgba(255,255,255,0.3)', lineHeight: 1.5 }}>From now on, payment is only possible with crypto.</div>
                        </div>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                        ...
                        <button id="_pwCryptoBtn" onClick={() => {
                            import('@/scripts/profile-logic').then(m => m._showPaywallCryptoPicker(Number(pw.amount) || 0, mid, document.getElementById('_pwWall')!));
                        }} style={{ width: '100%', padding: '18px', background: 'linear-gradient(135deg,#14081e,#0e0618)', border: '1px solid rgba(160,100,220,0.35)', borderRadius: 10, color: '#d4b0f0', fontFamily: 'Cinzel,serif', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '3px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, boxShadow: '0 8px 30px rgba(100,60,180,0.15)' }}>
        ...
        return (
            <div id="loading" style={{ height: '100dvh', width: '100vw', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', background: '#000', padding: '18vh 0 12vh', boxSizing: 'border-box' }}>
                ...
                <style>{`
                    ...
                    @keyframes _splashFade { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }
                `}</style>

                {/* Top — Welcome home */}
                <div style={{ fontFamily: 'Cinzel, serif', fontSize: '0.7rem', color: 'rgba(197,160,89,0.4)', letterSpacing: '6px', fontWeight: 500, animation: '_splashFade 0.8s ease-out both' }}>
                    WELCOME HOME
                </div>

                {/* Center — Avatar growing */}
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0 }}>
                    {_cachedAvatar ? (
                        <img
                            src={_cachedAvatar}
                            alt=""
                            style={{ width: 140, height: 140, borderRadius: '50%', objectFit: 'cover', border: '2px solid rgba(197,160,89,0.25)', animation: '_splashGrow 5s ease-out forwards, _splashGlow 2.5s ease-in-out infinite', marginBottom: 32 }}
                            onError={(e) => { e.currentTarget.style.display = 'none'; }}
                        />
                    ) : (
                        <div style={{ width: 140, height: 140, borderRadius: '50%', background: 'radial-gradient(circle, rgba(197,160,89,0.08) 0%, transparent 100%)', border: '2px solid rgba(197,160,89,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: '_splashGrow 5s ease-out forwards, _splashGlow 2.5s ease-in-out infinite', marginBottom: 32 }}>
                            <span style={{ fontFamily: 'Cinzel, serif', fontSize: '2.8rem', color: 'rgba(197,160,89,0.35)' }}>{_cachedName ? _cachedName[0].toUpperCase() : ''}</span>
                        </div>
                    )}
                    {/* Name — hero element */}
                    {_cachedName && (
                        <div style={{ fontFamily: 'Cinzel, serif', fontSize: '1.6rem', color: '#fff', letterSpacing: '3px', fontWeight: 700, animation: '_splashFade 0.8s ease-out 0.4s both', textAlign: 'center' }}>
                            {_cachedName}
                        </div>
                    )}
                </div>

                {/* Bottom — Handwritten subtitle */}
                <div style={{ fontFamily: 'Italianno, cursive', fontSize: '1.4rem', color: 'rgba(197,160,89,0.35)', animation: '_splashFade 0.8s ease-out 1s both' }}>
        ...
        <div id="paywallOverlay" style={{ display: 'none', position: 'fixed', inset: 0, zIndex: 999999, background: 'rgba(2,5,18,0.97)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
            <div style={{ maxWidth: 420, width: '100%', textAlign: 'center' }}>
                ...
                <div style={{ background: 'rgba(197,160,89,0.05)', border: '1px solid rgba(197,160,89,0.25)', borderRadius: 14, padding: '28px 24px', marginBottom: 28 }}>
                    <div style={{ fontFamily: 'Orbitron,sans-serif', fontSize: '0.38rem', color: 'rgba(197,160,89,0.45)', letterSpacing: '3px', marginBottom: 12 }}>MESSAGE FROM QUEEN KARIN</div>
                    <div id="paywallReason" style={{ fontFamily: 'Dancing Script,cursive', fontSize: '1.15rem', color: '#fff', lineHeight: 1.6 }}></div>
            ...
            <div id="paywallEmbedContainer" style={{ display: 'none', width: '100%', maxWidth: 480 }}>
                ...
                <div id="paywallPayError" style={{ fontFamily: 'Orbitron,sans-serif', fontSize: '0.4rem', color: '#ff4444', marginTop: 12, textAlign: 'center', display: 'none' }}></div>
            </div>
        </div>
            ...
            <div id="DESKTOP_APP">
                ...
                <div className="v-sidebar" style={{ backgroundColor: 'transparent', backdropFilter: 'blur(25px)' }}>
                    <div style={{ marginBottom: 40, textAlign: 'center', padding: '25px 15px', marginTop: 20, marginRight: 20, position: 'relative' }}>
                        ...
                        <button id="vaultLockBtn" onClick={() => (window as any).openVaultLockRequest?.()} style={{ marginTop: 15, width: '100%', background: 'rgba(139,0,0,0.06)', border: '1px solid rgba(139,0,0,0.3)', color: '#8b0000', fontFamily: 'Cinzel', fontSize: '0.65rem', letterSpacing: 3, padding: '12px 0', cursor: 'pointer', borderRadius: 8, fontWeight: 700, transition: 'all 0.2s' }} onMouseEnter={e => { e.currentTarget.style.background = 'rgba(139,0,0,0.12)'; e.currentTarget.style.borderColor = 'rgba(139,0,0,0.5)'; }} onMouseLeave={e => { e.currentTarget.style.background = 'rgba(139,0,0,0.06)'; e.currentTarget.style.borderColor = 'rgba(139,0,0,0.3)'; }}>
                            REQUEST LOCK
                        </button>
                ...
                <div id="viewServingTopDesktop" className="view-wrapper hidden" style={{ position: 'relative' }}>
                    ...
                    <div id="gridHero" className="v-card serve-grid-item" style={{ background: "url('/hero-bg.png')", backgroundSize: 'cover', backgroundPosition: 'center', minHeight: 'unset', display: 'flex', flexDirection: 'column', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
                        ...
                        <div id="kneelRewardOverlay" className="hidden" style={{ position: 'absolute', inset: 0, background: 'rgba(2,5,18,0.96)', zIndex: 5000, backdropFilter: 'blur(24px)', display: 'none', alignItems: 'center', justifyContent: 'center' }}>
                            <div style={{ width: '100%', padding: '16px 18px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0 }}>
                                {/* Crown icon */}
                                <div style={{ fontSize: '2rem', lineHeight: 1, marginBottom: 8, filter: 'drop-shadow(0 0 12px rgba(197,160,89,0.8))' }}>♛</div>
                                ...
                                {/* Subtitle */}
                                <div style={{ marginTop: 14, fontFamily: 'Orbitron', fontSize: '0.48rem', color: 'rgba(255,255,255,0.2)', letterSpacing: '2px', textAlign: 'center' }}>YOUR LOYALTY DOES NOT GO UNNOTICED</div>
                            </div>
                        </div>
                ...
                <button className="mob-nav-queen-btn" onClick={() => { (window as any).closeStandaloneTribute?.(); (window as any).closeExchequer?.(); (window as any).closeVaultLockOverlay?.(); (window as any).openMobChatOverlay(); }}>
                    ...
                    <div id="mobMsgBadge">
                        <svg viewBox="0 0 16 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <rect x="1" y="1" width="14" height="10" rx="1.5" stroke="#000" strokeWidth="1.2" fill="#c5a059"/>
                            <path d="M1.5 2L8 7L14.5 2" stroke="#000" strokeWidth="1.2" strokeLinecap="round"/>
                        </svg>
                    </div>
                </button>
                    ...
                    <div style={{ marginBottom: 28 }}>
                        ...
                        {enrolled.map((c: any) => {
                            ...
                            const taskName = taskIdx >= 0
                                ? ((pData?.challenge?.task_names || c.task_names || [])[taskIdx] || c.daily_task || null)
                                : (c.daily_task || null);
