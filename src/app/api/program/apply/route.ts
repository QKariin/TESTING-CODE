...
import { discordProgramLock } from '@/lib/discord';
...
// Actions: apply (with coins), apply-instant (lock now + video), apply-basic_program (already paid EUR)
export async function POST(req: Request) {
    try {
        ...
        if (!profile) {
            ...
            console.error('[PROGRAM APPLY] FAILED. searching:', email, 'uuid:', user.id, 'sample:', JSON.stringify(allProfiles));
        ...
        // ── BASIC PROGRAM FLOW (already paid EUR) ──
        if (action === 'apply-basic_program') {
            const params = profile.parameters || {};
            // Verify they have a valid basic_program purchase
            const khTier = params.basic_program_tier || params.daily_tier;
            if (!khTier) {
                return NextResponse.json({ error: 'No basic_program purchase found' }, { status: 400 });
            }

            const dailyDays = params.daily_days || tier.days;
            ...
            try { await _generateMemberProgram(session.id, memberId, dailyDays); } catch (e: any) { console.error('[program apply] Program generation failed:', e?.message); }

            // Clear the basic_program purchase marker
            delete params.basic_program_tier;
            delete params.basic_program_order;
            delete params.daily_tier;
            delete params.daily_days;
            delete params.daily_started;
            delete params.daily_expires;
            params.program_request = { sessionId: session.id, status: 'pending', requestedAt: new Date().toISOString() };
            await getAdmin().from('profiles').update({ parameters: params }).eq('ID', profile.ID);

            // Notify Queen
            _notifyQueen(profile.name || memberId, dailyDays, 'basic_program').catch(() => {});
        ...
        purchaseHistory.unshift({
            type: isInstant ? 'PROGRAM_LOCK_INSTANT' : 'PROGRAM_LOCK_REQUEST',
            amount: tier.coins,
            days: tier.days,
        ...
        try {
            await DbService.sendMessage(memberId,
                isInstant
                    ? `BASIC PROGRAM LOCK — ${tier.days} day sentence. ${tier.coins.toLocaleString()} coins charged. Submit your verification video to activate.`
                    : `BASIC PROGRAM LOCK REQUESTED — ${tier.days} days. ${tier.coins.toLocaleString()} coins held. Waiting for Queen Karin's approval.`,
                'system');
        } catch (_) {}
        ...
        // try { await getAdmin().from('global_messages').insert({ sender_email: 'system', sender_name: 'SYSTEM', sender_avatar: null, message: `PROGRAM_LOCK_CARD::${JSON.stringify(cardData)}` }); } catch (_) {}
        // discordProgramLock(memberName, tier.days, isInstant ? 'instant' : 'request').catch(() => {});
...
// GET /api/program/apply — check current program request status
export async function GET(req: Request) {
    try {
        ...
        // If session is active but profile missing program markers, fix the profile (not the session)
        const profileParams = getProfile?.parameters || {};
        if (session.status === 'active' && !profileParams.active_overlay && getProfile) {
            try {
                const params = { ...profileParams, active_overlay: 'program' };
                if (!params.program_request) params.program_request = { sessionId: session.id, status: 'active' };
...
async function _generateMemberProgram(sessionId: string, memberId: string, totalDays = 30) {
    ...
    await admin.from('program_member').delete().eq('member_id', memberId);
    const program: Record<string, any[]> = {};
    try {
        const { data: template, error: tplErr } = await admin
            .from('program_template').select('*').order('day_number');
        if (tplErr) console.error('[program apply] Template read error:', tplErr.message);
        if (template && template.length > 0) {
            console.log(`[program apply] Copying ${template.length} template days for ${memberId}`);
            for (const row of template) {
                program[String(row.day_number)] = typeof row.tasks === 'string' ? JSON.parse(row.tasks) : row.tasks;
            }
        }
    } catch (e: any) {
        console.error('[program apply] Template read failed:', e?.message);
    }
    ...
    await admin.from('program_member').insert({
...
async function _notifyQueen(name: string, days: number, type: string) {
    const titles: Record<string, string> = {
        ...
        basic_program: 'BasicProgram Lock',
    };
