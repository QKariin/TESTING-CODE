...
// POST /api/program/proof — submit video proof for self-lock
export async function POST(req: Request) {
    try {
        ...
        // Set active_overlay = 'program' on profile
        try {
            ...
            if (fullProfile) {
                ...
                params.program_request = { ...params.program_request, status: 'active', activatedAt: now };
        ...
        try {
            let { data: prog } = await supabaseAdmin
                .from('program_member').select('id, program').eq('session_id', sessionId).maybeSingle();

            // If no program exists yet (payment step failed), generate now
            if (!prog) {
                console.log(`[program proof] No program found for session ${sessionId}, generating fresh...`);
            ...
            if (day1 && !day1.some((t: any) => t.config)) {
                console.log(`[program proof] Program is stale for session ${sessionId}, regenerating...`);
                const freshProgram = generateDefaultProgram();
                try {
                    const { data: template } = await supabaseAdmin
                        .from('program_template').select('*').order('day_number');
                ...
                if (prog.id) {
                    await supabaseAdmin.from('program_member').update({ program: JSON.stringify(freshProgram) }).eq('id', prog.id);
                }
            ...
            console.error('[program proof] Day 1 orders creation failed:', e?.message);
        }

        // Video proof counts as day 1 daily check (pending Queen's approval)
        try {
            const today = new Date().toISOString().split('T')[0];
            ...
            console.error('[program proof] Day 1 daily check insert failed:', e?.message);
        }
        ...
        console.error('[PROGRAM PROOF] Error:', err);
