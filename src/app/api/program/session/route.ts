...
export async function POST(req: NextRequest) {
    ...
    if (action === 'complete_order') {
        ...
        try { await DbService.awardPoints(email, 10); } catch (_) {}
    ...
    if (action === 'approve_task') {
        ...
        try {
            const cardData = { status: 'approve', points: 25, type: 'task', comment: comment || null, taskText: sub?.label || sub?.order_type || 'Task', thumbnail: sub?.photo_url || null };
...
async function _updateOrderDone(sessionId: string, date: string, orderType: string, amount?: number) {
    ...
    if (perfect && !daily.perfect) {
        ...
        if (session) {
            ...
            // Perfect day bonus — sync to profile score (leaderboard)
            if (session.member_id) {
                try { await DbService.awardPoints(session.member_id, 50); } catch (_) {}
            }
        }
    }
}
