...
export async function POST(req: Request) {
    ...
    try {
        ...
        // Sync kneel count to program_daily orders (so kneeling counts toward perfect day in program)
        try {
            const emailLc2 = (taskEmail || memberId).toLowerCase();
            const { data: progSession } = await supabaseAdmin.from('program_sessions')
                .select('id').eq('member_id', emailLc2).eq('status', 'active')
                .order('started_at', { ascending: false }).limit(1).maybeSingle();
            if (progSession) {
                const { data: daily } = await supabaseAdmin.from('program_daily')
                    .select('id, orders, orders_completed, orders_total')
                    .eq('session_id', progSession.id).eq('date', todayStr).maybeSingle();
                if (daily) {
                    const orders: any[] = typeof daily.orders === 'string' ? JSON.parse(daily.orders) : (daily.orders || []);
                    let changed = false;
                    for (const o of orders) {
