...
// =========================================
// CLEAN TRIBUTE SYSTEM (GRID ONLY)
// =========================================

// 1. MAIN TOGGLE (Smart Targeting)
window.toggleTributeHunt = function() {
    const isMobile = window.innerWidth <= 768;
    
    // Target the specific overlay based on device
    // Mobile = Look inside #MOBILE_APP. Desktop = Look for standard ID.
    ...
    if (!overlay) { console.error("Tribute Overlay not found."); return; }

    if (overlay.classList.contains('hidden')) {
        // OPEN
        overlay.classList.remove('hidden');
        renderTributeGrid(grid); // Build the grid
    } else {
        // CLOSE
        overlay.classList.add('hidden');
        if(isMobile && window.resetMobileFooter) window.resetMobileFooter();
    }
};

// 2. RENDERER (Builds the 2-Column List)
function renderTributeGrid(gridContainer) {
    if (!gridContainer) return;
    gridContainer.innerHTML = ""; // Wipe clean
    ...
    window.WISHLIST_ITEMS.forEach(item => {
        ...
        el.className = 'store-item'; // CSS handles the layout
        ...
        el.onclick = () => window.selectTributeItem(item, el);
        gridContainer.appendChild(el);
    });
}

// 3. SELECTOR (Hijacks Footer on Mobile)
window.selectTributeItem = function(item, element) {
    // Visual Highlight
    ...
    if (isMobile) {
        // Change Chat Input to Confirm Button
        ...
        // Desktop Instant Buy (Optional)
        if(confirm(`Sacrifice ${item.price} coins for ${item.name}?`)) {
            window.confirmPurchase(item.name, item.price);
        }
    }
};

// 4. BUYER (Transaction Logic)
window.confirmPurchase = function(name, price) {
    ...
    // Send to Database
    window.parent.postMessage({ 
        type: "PURCHASE_ITEM", itemName: name, cost: price, 
...
// 5. CLEANUP (Restores Chat Input)
window.resetMobileFooter = function() {
    const inputWrap = document.querySelector('#mobChatSection .chat-input-wrapper');
    const sendBtn = document.querySelector('#mobChatSection .chat-btn-send');
...
function updateStats() {
    ...
    if (!subName || !userProfile || !gameStats) return; lets s
