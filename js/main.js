...
// =========================================
// TRIBUTE SYSTEM (SMART TARGETING)
// =========================================
...
window.toggleTributeHunt = function() {
    ...
    // 1. SMART TARGETING
    // If on Mobile, force it to find the ID inside #MOBILE_APP
    // If on Desktop, use the standard ID
    ...
    // Safety: If we can't find the box, stop.
    if (!overlay) { console.error("Tribute Overlay not found."); return; }

    // 2. TOGGLE LOGIC
    if (overlay.classList.contains('hidden')) {
        overlay.classList.remove('hidden');
        renderTributeGrid(grid); // Use the Grid renderer we wrote
    } else {
        overlay.classList.add('hidden');
