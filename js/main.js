...
function updateStats() {
    ...
    const hName = document.getElementById('mob_slaveName');
    const hRank = document.getElementById('mob_slaveRank');
    const hPoints = document.getElementById('mob_slavePoints');
    const hCoins = document.getElementById('mob_slaveCoins');
    const hSpent = document.getElementById('mob_slaveSpent');

    if (hName) hName.innerText = userProfile.name || "Slave";
    if (hRank) hRank.innerText = visualRank.toUpperCase();
    if (hPoints) hPoints.innerText = (gameStats.points || 0).toLocaleString();
    if (hCoins) hCoins.innerText = (gameStats.coins || 0).toLocaleString();
    if (hSpent) hSpent.innerText = (gameStats.totalSpent || 0).toLocaleString();
