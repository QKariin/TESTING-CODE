...
// =========================================
// TRIBUTE SYSTEM (MOBILE FIX)
// =========================================

window.toggleTributeHunt = function() {
    const isMobile = window.innerWidth <= 768;
    
    // 1. FIND THE CORRECT OVERLAY
    // Critical: On mobile, we MUST look inside #MOBILE_APP or it grabs the wrong hidden div.
    ...
    if (overlay.classList.contains('hidden')) {
        overlay.classList.remove('hidden');
        renderTributeGrid(grid); // Populate items
    } else {
        overlay.classList.add('hidden');
        if(isMobile) window.resetMobileFooter(); // Put the chat input back
    }
};
