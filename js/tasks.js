...
export function getRandomTask() {
    // PREVIEW MODE: Provide a mock task for testing
    if (window.location.protocol === "file:" ||
        ...
        const mockTask = {
            text: "Write a 500-word essay on why Queen Karin is the supreme ruler of the realm",
            category: 'general',
            timestamp: Date.now()
        };
        setCurrentTask(mockTask);

        const endTimeVal = Date.now() + 3600000; // 1 hour from now
        const newPendingState = { task: mockTask, endTime: endTimeVal, status: "PENDING" };
...
export function cancelPendingTask() {
    ...
    // PREVIEW MODE: Allow free skip for testing
    if (window.location.protocol === "file:" ||
        window.location.hostname === "localhost" ||
        window.location.hostname === "127.0.0.1" ||
        window.location.hostname === "") {
        console.log("⚠️ PREVIEW MODE: Free skip allowed");
        finishTask(false);
        return;
    }
