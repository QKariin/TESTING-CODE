...
export async function renderGallery() {
    ...
    // 4c. Inject Official Streaks into Routine Label (Mirror Only)
    ...
    if (routineContainer) {
        ...
        if (label) {
            const curr = (gameStats && (gameStats.routine_streak || gameStats.routinestreak)) || 0;
            const best = (gameStats && gameStats.bestRoutinestreak) || 0;
            label.innerHTML = `DAILY ROUTINES <span style="float:right; opacity:0.7; font-size:0.4rem; letter-spacing:1px;">CURR: ${curr} | BEST: ${best}</span>`;
        }
    }
