// Chat functionality - RAW MODE (Bypasses CPU Quota & Fixes Wix)
import {
...
function getSafeSrc(rawUrl) {
    ...
    // 1. WIX DATABASE FIX (Extract ID from wix:image://v1/ID/name)
    if (rawUrl.includes('wix:image') || rawUrl.includes('wix:video')) {
        const parts = rawUrl.split('/');
        for (let i = 0; i < parts.length; i++) {
            // Find the 'v1' segment, the ID is usually immediately after
            if (parts[i] === 'v1' && parts[i+1]) {
                ...
                // Use generic Wix media URL
                return `https://static.wixstatic.com/media/${id}`;
            }
        }
    }

    // 2. BYTESCALE FIX (CPU QUOTA ERROR)
    // We MUST use /raw/ to avoid the "Transformation CPU quota used" error
    if (rawUrl.includes('upcdn.io')) {
        ...
        // Remove ANY query parameters (?w=, ?f=) because they trigger the CPU limit
        return clean.split('?')[0]; 
    }

    // 3. FALLBACK FOR STANDARD LINKS
        ...
        if (m.message && (isUrl || m.message.startsWith('WISHLIST::'))) {
            ...
            // B. IMAGES / VIDEO
                ...
                if (isVideo) {
                    ...
                    // ERROR HANDLER: If image breaks (404/403), show a text link instead of a broken icon
                    contentHtml = `<div class="msg ${msgClass}" style="padding:0;">
                        <img src="${srcUrl}" 
                             ...
                             onerror="this.style.display='none'; this.parentElement.innerHTML='<a href=\\'${srcUrl}\\' target=\\'_blank\\' style=\\'color:#c5a059; padding:10px; display:block; text-align:center; font-size:0.7rem;\\'>[VIEW IMAGE]</a>'">
                    </div>`;
                }
            }
        }
