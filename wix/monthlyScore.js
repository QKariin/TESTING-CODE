...
export const invoke = async ({ payload }) => {
  ...
  // Sort by monthlyScore descending and take top 3
  ...
  const entry = {
      ...
      score1: topThree[0]?.monthlyScore || 0,
      name2: topThree[1]?.title_fld || null,
      score2: topThree[1]?.monthlyScore || 0,
      name3: topThree[2]?.title_fld || null,
      score3: topThree[2]?.monthlyScore || 0,
  };

  // Insert the single row into monthlyLeaderboard
  await wixData.insert("MonthlyLeaderboard", entry);
  
  // Reset monthlyScore to 0 for each valid item
