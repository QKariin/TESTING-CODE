...
-- 4. Program check log — proper table for daily checks + all program submissions
-- Same pattern as user_routines: one row per submission, proper columns, no JSON
CREATE TABLE IF NOT EXISTS program_check_log (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    session_id UUID NOT NULL REFERENCES program_sessions(id) ON DELETE CASCADE,
    member_id TEXT NOT NULL,
    date DATE NOT NULL,
    type TEXT NOT NULL DEFAULT 'daily_check',
    proof_url TEXT,
...
CREATE INDEX IF NOT EXISTS idx_program_check_log_session ON program_check_log(session_id, date);
CREATE INDEX IF NOT EXISTS idx_program_check_log_status ON program_check_log(status);
CREATE INDEX IF NOT EXISTS idx_program_check_log_member ON program_check_log(member_id, date);

-- 5. Program submissions — proper table for all task submissions (not daily, that's program_check_log)
...
CREATE INDEX IF NOT EXISTS idx_program_submissions_session_date ON program_submissions(session_id, date);
CREATE INDEX IF NOT EXISTS idx_program_submissions_status ON program_submissions(status);
CREATE INDEX IF NOT EXISTS idx_program_submissions_member ON program_submissions(member_id, date);

-- Indexes
CREATE INDEX IF NOT EXISTS idx_program_member_session ON program_member(session_id);
CREATE INDEX IF NOT EXISTS idx_program_member_member ON program_member(member_id);
CREATE INDEX IF NOT EXISTS idx_program_template_day ON program_template(day_number);

-- Seed initial spin wheel options
INSERT INTO program_config (key, value) VALUES ('spin_wheel', '[
...
INSERT INTO program_config (key, value) VALUES ('card_deck', '[
...
-- Seed quiz questions
INSERT INTO program_config (key, value) VALUES ('quiz_questions', '[
    ...
    {"question": "What is the purpose of the daily check?", "answer": "To prove devotion and submission"},
...
-- Seed lines texts
INSERT INTO program_config (key, value) VALUES ('lines_texts', '[
    "I am owned property",
...
INSERT INTO program_config (key, value) VALUES ('body_writing', '[
    "OWNED",
    "PROPERTY",
    "DENIED",
    "LOCKED",
...
INSERT INTO program_config (key, value) VALUES ('exercises', '[
