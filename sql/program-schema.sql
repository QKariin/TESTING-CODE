-- ============================================================
-- PROGRAM (BASIC PROGRAM) SYSTEM SCHEMA
-- Run this in Supabase SQL Editor
-- ============================================================

-- 1. PROGRAM SESSIONS
-- One row per lock period. Created when member buys basic_program access.
...
CREATE TABLE IF NOT EXISTS program_daily (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE TABLE IF NOT EXISTS program_begs (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    session_id UUID NOT NULL REFERENCES program_sessions(id) ON DELETE CASCADE,
    member_id TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS program_tributes (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    session_id UUID NOT NULL REFERENCES program_sessions(id) ON DELETE CASCADE,
    member_id TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS program_spins (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    session_id UUID NOT NULL REFERENCES program_sessions(id) ON DELETE CASCADE,
    member_id TEXT NOT NULL,
    date DATE NOT NULL,
...
-- 7. PROGRAM LEADERBOARD (cached view)
...
CREATE TABLE IF NOT EXISTS program_attention (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS idx_program_sessions_member_active
    ON program_sessions(member_id, status) WHERE status = 'active';

CREATE INDEX IF NOT EXISTS idx_program_daily_session
    ON program_daily(session_id, day_number);

CREATE INDEX IF NOT EXISTS idx_program_daily_member_date
    ON program_daily(member_id, date);

CREATE INDEX IF NOT EXISTS idx_program_trials_session
    ON program_trials(session_id, date);

CREATE INDEX IF NOT EXISTS idx_program_trials_pending
    ON program_trials(status) WHERE status = 'submitted';

CREATE INDEX IF NOT EXISTS idx_program_begs_pending
    ON program_begs(status) WHERE status = 'pending';

CREATE INDEX IF NOT EXISTS idx_program_tributes_session
    ON program_tributes(session_id);

CREATE INDEX IF NOT EXISTS idx_program_leaderboard_rank
    ON program_leaderboard(rank);

CREATE INDEX IF NOT EXISTS idx_program_attention_member
    ON program_attention(member_id, requested_at DESC);

CREATE INDEX IF NOT EXISTS idx_program_attention_pending
    ON program_attention(completed) WHERE completed = FALSE;

CREATE INDEX IF NOT EXISTS idx_program_adjustments_session
    ON program_adjustments(session_id, created_at);

-- ============================================================
-- END OF PROGRAM SYSTEM SCHEMA
-- ============================================================
