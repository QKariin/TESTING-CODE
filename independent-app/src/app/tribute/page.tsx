        ...
        <div style={{ background: '#020512', color: '#fff', overflowX: 'hidden' }}>
            ...
            {!challengeDismissed && (
                <div style={{ position: 'fixed', bottom: 24, left: 12, right: 12, zIndex: 10000002, background: 'rgba(5,8,18,0.97)', border: '1px solid rgba(74,222,128,0.4)', borderRadius: 14, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 12, backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', maxWidth: 640, margin: '0 auto' } as React.CSSProperties}>
                    ...
                    <button onClick={handleTribute} disabled={loading} style={{ background: 'linear-gradient(135deg,#4ade80,#16a34a)', borderRadius: 8, padding: '5px 12px', fontFamily: 'Orbitron,sans-serif', fontSize: '0.38rem', color: '#000', fontWeight: 700, border: 'none', cursor: 'pointer', letterSpacing: '1px', flexShrink: 0 }}>JOIN</button>
                    <button onClick={() => setChallengeDismissed(true)} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, padding: '5px 10px', color: 'rgba(255,255,255,0.4)', fontFamily: 'Orbitron,sans-serif', fontSize: '0.36rem', cursor: 'pointer', flexShrink: 0, letterSpacing: '1px' }}>DISMISS</button>
                </div>
            )}
