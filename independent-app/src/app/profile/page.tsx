...
// Called after successful 2s hold — updates state, shows reward, saves to DB
async function completeKneelFromPage(profile: any) {
    const { setState: setKS, getState: getKS } = await import('@/scripts/profile-state');
    setKS({ isLocked: true, lastWorshipTime: Date.now() });

    // Update UI to locked state
    const fill = document.getElementById('heroKneelFill');
    const txt = document.getElementById('heroKneelText');
    const mFill = document.getElementById('mob_kneelFill');
    const mTxt = document.querySelector('.kneel-label') as HTMLElement | null;
    const mBar = document.querySelector('.mob-kneel-zone') as HTMLElement | null;
    if (fill) { fill.style.transition = 'none'; fill.style.width = '100%'; }
    if (txt) txt.innerText = 'LOCKED: 60m';
    if (mFill) { mFill.style.transition = 'none'; mFill.style.width = '100%'; }
    if (mTxt) mTxt.innerText = 'LOCKED: 60m';
    if (mBar) mBar.style.borderColor = '#ff003c';
    ...
    overlay.innerHTML = `
        ...
        <div style="font-family:'Cinzel';color:rgba(255,255,255,0.5);font-size:0.8rem;letter-spacing:2px;">Choose your reward</div>
        <div style="display:flex;gap:16px;margin-top:10px;">
            <button id="_rMerit" style="padding:14px 28px;background:rgba(197,160,89,0.1);border:1px solid #c5a059;color:#c5a059;font-family:'Orbitron',sans-serif;font-size:0.75rem;letter-spacing:2px;cursor:pointer;border-radius:4px;">+50 MERIT</button>
            <button id="_rCoins" style="padding:14px 28px;background:rgba(0,150,255,0.08);border:1px solid #4af;color:#4af;font-family:'Orbitron',sans-serif;font-size:0.75rem;letter-spacing:2px;cursor:pointer;border-radius:4px;">+10 COINS</button>
        </div>
        <div style="color:rgba(255,255,255,0.2);font-size:0.6rem;margin-top:4px;">Next kneel available in 60 minutes</div>
    `;
    document.body.appendChild(overlay);
    const close = () => overlay.remove();
    document.getElementById('_rMerit')!.onclick = () => { close(); };
    document.getElementById('_rCoins')!.onclick = () => { close(); };
...
export default function ProfilePage() {
    ...
    useEffect(() => {
        ...
        const kneelButtons = [
            ...
            document.getElementById('mobKneelBar'),
        ...
        const onStart = (e: Event) => {
            if (holdTimer) return; // already timing — ignore double-fire (touchstart + mousedown on mobile)
            ...
            const mTxt = document.querySelector('.kneel-label') as HTMLElement | null;
            const mBar = document.querySelector('.mob-kneel-zone') as HTMLElement | null;
            if (fill) { fill.style.transition = `width ${HOLD_MS}ms linear`; fill.style.width = '100%'; }
            if (txt) txt.innerText = 'KNEELING...';
            if (mFill) { mFill.style.transition = `width ${HOLD_MS}ms linear`; mFill.style.width = '100%'; }
            if (mTxt) mTxt.innerText = 'SUBMITTING...';
            if (mBar) mBar.style.borderColor = 'var(--gold, #c5a059)';

            holdTimer = setTimeout(() => {
                holdTimer = null;
                // Trigger completion via the kneeling module
                (window as any).handleHoldStart && completeKneelFromPage(profile);
            }, HOLD_MS);
        };

        const onEnd = (e: Event) => {
            if (!holdTimer) return; // already completed or never started
            ...
            if (txt) txt.innerText = 'HOLD TO KNEEL';
            if (mFill) { mFill.style.transition = 'width 0.3s ease'; mFill.style.width = '0%'; }
            if (mTxt) mTxt.innerText = 'HOLD TO KNEEL';
            if (mBar) mBar.style.borderColor = '#c5a059';
        };

        kneelButtons.forEach(btn => {
